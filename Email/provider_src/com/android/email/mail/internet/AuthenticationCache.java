...
public class AuthenticationCache {
    ...
    private CacheEntry getEntry(Context context, Account account) {
        ...
        if (account.isSaved() && !account.isTemporary()) {
            ...
            final HostAuth hostAuth = account.getOrCreateHostAuthRecv(context);
            final Credential credential = hostAuth.getOrCreateCredential(context);//UNISOC: Modify for bug 1233006
            entry = new CacheEntry(account.mId, credential.mProviderId, credential.mAccessToken,
                    credential.mRefreshToken, credential.mExpiration);
        }
        return entry;
    ...
    /* SPRD: Modify for bug776307 @{ */
    private void saveEntry(Context context, CacheEntry entry) throws MessagingException {
        ...
        if (account != null) {
            ...
            LogUtils.w(Logging.LOG_TAG, "saveEntry query account get null", new Throwable());
    ...
    private void clearEntry(Context context, CacheEntry entry) {
        ...
        try {
            ...
            LogUtils.e(Logging.LOG_TAG, "saveEntry failed");
        }
        /* @} */
