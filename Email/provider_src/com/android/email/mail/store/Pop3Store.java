...
public class Pop3Store extends Store {
    ...
    public class Pop3Folder extends Folder {
        ...
        public void fetchBody(Pop3Message message, int lines,
                EOLConvertingInputStream.Callback callback) throws IOException, MessagingException, MimeException {
        ...
        /* UNISOC: Modify for bug1011107 {@ */
        public void fetchEnvelope(Pop3Message[] messages) throws IOException, MessagingException {
            ...
            if (unsizedMessages != 0) {
                if (unsizedMessages >= 50 || mMessageCount <= 5000) {
                    HashSet<String> msgUidIndex = new HashSet();
                    for (Pop3Message message : messages) {
                    ...
                    while ((response = mTransport.readLine(false)) != null) {
                        ...
                        try {
                            ...
                            mSize = Integer.parseInt(parts[1]);
                            message = (Pop3Message) mMsgNumToMsgMap.get(msgNum);
                        } catch (NumberFormatException nfe) {
                            throw new IOException();
                        } catch (Throwable th) {
                            LogUtils.w(Logging.LOG_TAG, "Can't execute LIST Command");
                        }
                        if (message != null && msgUidIndex.contains(message.getUid())) {
                            message.setSize(mSize);
                        }
                    }
                } else {
                    for (int msgNum = 0; msgNum < messages.length; msgNum++) {
                        Pop3Message message = messages[msgNum];
                        int messageId = mUidToMsgNumMap.get(message.getUid());
                        try {
                            String response = executeSimpleCommand(String.format(Locale.US, "LIST %d", messageId));
                            message.setSize(Integer.parseInt(response.split(SEPARATOR)[2]));
                        } catch (NumberFormatException nfe) {
                            LogUtils.w(Logging.LOG_TAG, "Can't execute LIST %d Command" + messageId);
                            throw new IOException();
                        }
                    }
                }
