...
public class ImapStore extends Store {
    ...
    static void createHierarchy(HashMap<String, ImapFolder> mailboxes) {
        /* UNISOC: Modify for bug1208566 {@ */
        for (Entry<String, ImapFolder> temEntry : mailboxes.entrySet()) {
            String path = (String) temEntry.getKey();
            Mailbox mailbox = ((ImapFolder) temEntry.getValue()).mMailbox;
    ...
    public Folder[] updateFolders() throws MessagingException {
        ...
        try {
            ...
            /* SPRD: Modify for bug508474 {@ */
            final ImapFolder newFolder = addMailbox(
                    mContext, mAccount.mId, inbox != null ? inbox.mServerId : ImapConstants.INBOX, '\0', true /*selectable*/, inbox);
            /* @} */
