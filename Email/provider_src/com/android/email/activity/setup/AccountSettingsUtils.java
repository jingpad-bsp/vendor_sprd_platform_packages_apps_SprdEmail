...
public class AccountSettingsUtils {
    ...
    public final static String FROM_ACCOUNT_ADD = "FromAccountAdd";
    public final static String FROM_OTHER_ADD = "FromOtherAdd";
    ...
    public static void commitSettings(Context context, Account account) {
        if (!account.isSaved()) {
            ...
            /* SPRD: Modify for bug495239 bug674599 {@ */
            final SharedPreferences statePrefs = context.getSharedPreferences(
                    QuickResponsesCursorAdapter.QUICK_RESPONSE_PREF_STATE + account.getEmailAddress(),
            ...
            for (String quickResponse: defaultQuickResponses) {
                ...
                if (!TextUtils.isEmpty(quickResponse)) {
                    ...
                    stateEditor.putString(quickResponse, "unchanged"+position);
                }
                position++;
            }
            stateEditor.commit();
   ...
   /**
    ...
    public static String inferServerName(Context context, String server, String incoming,
        ...
        if (firstDotIndex != -1) {
            ...
            String firstWord = server.substring(0, firstDotIndex).toLowerCase(Locale.ROOT); //UNISOC: Modify for bug1192114
            String[] hostPrefixes =
    ...
    /*
     * SPRD:bug471289, 716355 use XML file customization for server address {@
     */
    public static String findEspeciallyHostName(Context context, String domain, String protocol) {
        String hostname = "";
        try {
            XmlResourceParser xml = context.getResources().getXml(R.xml.providers_manual);
            int xmlEventType;
            ...
            while ((xmlEventType = xml.next()) != XmlResourceParser.END_DOCUMENT) {
                if (xmlEventType == XmlResourceParser.START_TAG
                    ...
                    String providerDomain = getXmlAttribute(context, xml, "domain");
                    try {
                        if (matchProviderForEspecially(domain, providerDomain)) {
                            if (protocol.toLowerCase().contains("smtp")) {
                                hostname = getXmlAttribute(context, xml, "smtp");
                            } else if (protocol.toLowerCase().contains("pop")) {
                                hostname = getXmlAttribute(context, xml, "pop");
                            } else if (protocol.toLowerCase().contains("imap")) {
                                hostname = getXmlAttribute(context, xml, "imap");
                            } else if (protocol.toLowerCase().contains("exchange")) {
                                hostname = getXmlAttribute(context, xml, "eas");
                            }
                            break;
                        }
                    } catch (IllegalArgumentException e) {
            ...
            LogUtils.e(Logging.LOG_TAG, "Error while trying to find special hostname", e);
        }

        return hostname;
    }

    public static boolean matchProviderForEspecially(String testDomain, String providerDomain) {
        String[] testParts = testDomain.split(DOMAIN_SEPARATOR);
        String[] providerParts = providerDomain.split(DOMAIN_SEPARATOR);
        if (testParts.length < 2 || providerParts.length < 2) {
            return false;
        }

        if(testParts[testParts.length-1].toLowerCase().equals(
                providerParts[providerParts.length-1].toLowerCase())
                && testParts[testParts.length-2].toLowerCase().equals(
                providerParts[providerParts.length-2].toLowerCase())) {
            return true;
        }
        return false;
    }
    ...
    /**
     * SPRD: 523599 Add for auto match emailAddress when set up account.@{
     */
    private static boolean hasWildchars(String providerDomain) {
        String[] providerParts = providerDomain.split(DOMAIN_SEPARATOR);
        for (int i = 0; i < providerParts.length; i++) {
            String providerPart = providerParts[i].toLowerCase();
            if (providerPart.equals(WILD_STRING) || providerPart.startsWith("?")) {
                return true;
            }
        }
        return false;
    ...
    public static ArrayList<String> collectAutoSetupDomain(Context context) {
        ...
        try {
            ...
            LogUtils.e(Logging.LOG_TAG,
                    "Catch NotFoundException while trying to load manual_setup_helper settings.", e);
        } catch (XmlPullParserException ex) {
            LogUtils.e(
                    Logging.LOG_TAG,
                    "Catch XmlPullParserException while trying to load manual_setup_helper settings.",
                    ex);
        } catch (IOException io) {
            LogUtils.e(Logging.LOG_TAG,
                    "Catch IOException while trying to load manual_setup_helper settings.", io);
        }
        // Add a '@' for each domain
        ArrayList<String> atServerName = new ArrayList<String>();
        Iterator<String> itr = serverName.iterator();
        while (itr.hasNext()) {
            atServerName.add("@" + itr.next());
        }
        return atServerName;
    }

    /**
     * Check whether the given domain string is valid Domain is configured by
     * Vendor, and one '.' is necessary and no '@' should exists
     *
     * @param domain
     * @return
     */
    ...
    /* SPRD modify for bug712291 {@ */
    public static void saveAddressToHistoryDB(Context context, List<String> addressList,
            String type) {
        if (!addressList.isEmpty()) {
            new SaveAddressTask(context, type).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR,
                    addressList);
        }
    }

    /**
     * SaveAddressTask for saving address outside of the UI thread
     */
    static class SaveAddressTask extends AsyncTask<List<String>, Void, Void> {
        private Context mContext;
        private String mType;

        public SaveAddressTask(Context context, String type) {
            mContext = context;
            mType = type;
        }
        ...
        protected Void doInBackground(List<String>... params) {
            EmailAddress.saveAddressList(mContext, params[0], mType);
            return null;
        }
    }
    /* @} */
