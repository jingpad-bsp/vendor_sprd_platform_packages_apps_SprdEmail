...
public class ImapService extends Service {
    ...
    public static synchronized int synchronizeMailboxSynchronous(Context context,
        ...
        try {
            ...
            /* SPRD modify for bug701108 {@ */
            LogUtils.i(Logging.LOG_TAG, "synchronizeMailboxSynchronous", e);
            /* @} */
                        ...
                        try {
                            ...
                            if (localMessage != null) {
                                try {
                                    ...
                                    LegacyConversions.updateMessageFields(context, localMessage,
...
//            final long queryEndDate = endDate - DateUtils.DAY_IN_MILLIS - DateUtils.HOUR_IN_MILLIS;
        ...
        /* SPRD modify for bug701108 {@ */
        HashMap<Long, LocalMessageInfo> localUpdatedOldMessageMap = new HashMap<Long, LocalMessageInfo>();
        Cursor updates = null;
        try {
            updates = resolver.query(
            ...
            while (updates.moveToNext()) {
                LocalMessageInfo info = new LocalMessageInfo(updates);
                // If the message has no server id, it's local only. This should only happen for
                // mail created on the client that has failed to upsync. We want to ignore such
                // mail during synchronization (i.e. leave it as-is and let the next sync try again
                // to upsync).
                localUpdatedOldMessageMap.put(info.mId, info);
            }
        } finally {
            if (updates != null) {
                updates.close();
        ...
        LogUtils.i(Logging.LOG_TAG,
                "synchronizeMailboxGeneric localMessageInfo.remoteSupportsFlagged:"
                        + remoteSupportsFlagged + " remoteSupportsSeen: " + remoteSupportsSeen
                        + " remoteSupportsAnswered:" + remoteSupportsAnswered
                        + " remoteMessages size:" + remoteMessages.length);
        /* @} */
        ...
        if (remoteSupportsSeen || remoteSupportsFlagged || remoteSupportsAnswered) {
            for (Message remoteMessage : remoteMessages) {
                ...
                /* SPRD modify for bug701108 {@ */
                if (localUpdatedOldMessageMap.containsKey(localMessageInfo.mId)) {
                    LocalMessageInfo updatedInfo = localUpdatedOldMessageMap
                            .get(localMessageInfo.mId);
                    boolean updatedOldSeen = updatedInfo.mFlagRead;
                    boolean updatedOldFlagged = updatedInfo.mFlagFavorite;
                    int updatedOldFlags = updatedInfo.mFlags;
                    boolean updatedOldAnswered = (updatedOldFlags & EmailContent.Message.FLAG_REPLIED_TO) != 0;

                    LogUtils.i(Logging.LOG_TAG,
                            "synchronizeMailboxGeneric message Has been Updated locally"
                                    + " updatedOldSeen:" + updatedOldSeen
                                    + " updatedOldFlagged: " + updatedOldFlagged
                                    + " updatedOldFlags :" + updatedOldFlags
                                    + " updatedOldAnswered:" + updatedOldAnswered
                                    + " remoteSeen:" + remoteSeen
                                    + " remoteFlagged:" + remoteFlagged
                                    + " remoteAnswered : " + remoteAnswered
                                    + " localSeen:" + localSeen
                    ...
                    newSeen = newSeen && (updatedOldSeen != remoteSeen);
                    newFlagged = newFlagged && (updatedOldFlagged != remoteFlagged);
                    newAnswered = newAnswered && (updatedOldAnswered != remoteAnswered);
                }
    ...
    private static void processPendingDeletesSynchronous(Context context, Account account,
        ...
        try {
            ...
            /* SPRD modify for bug701108 {@ */
            LogUtils.i(Logging.LOG_TAG, "Unable to process pending delete for id="
                    + lastMessageId + ": " + me);
            /* @} */
        } finally {
            deletes.close();
            ...
            while (mailboxes.moveToNext()) {
                ...
                try {
                    while (upsyncs1.moveToNext()) {
                        ...
                        /* SPRD modify for bug701108 {@ */
                        LogUtils.i(Logging.LOG_TAG, "processPendingUploadsSynchronous upsyncs1 id:"
                                + id);
                        /* @} */
                        lastMessageId = id;
    ...
    private static void processPendingUpdatesSynchronous(Context context, Account account,
        ...
        try {
            ...
            while (updates.moveToNext()) {
                ...
                /* SPRD modify for bug701108 {@ */
                LogUtils.i(Logging.LOG_TAG, "processPendingUpdatesSynchronous oldMessage:"
                        + oldMessage.mId);
                /* @} */

                if (newMessage != null) {
                    /* SPRD modify for bug701108 {@ */
                    LogUtils.i(Logging.LOG_TAG, "processPendingUpdatesSynchronous newMessage:"
                            + newMessage.mId);
                    /* @} */
                ...
                /* SPRD modify for bug701108 {@ */
                LogUtils.i(Logging.LOG_TAG, "processPendingUpdatesSynchronous changeFlagged:"
                        + changeFlagged + " changeRead:" + changeRead);
                /* @} */
            ...
            // Presumably an error here is an account connection failure, so there is
            // no point in continuing through the rest of the pending updates.
            LogUtils.i(Logging.LOG_TAG, "Unable to process pending update for id="
                    + lastMessageId + ": " + me);
        } finally {
            updates.close();
        }
