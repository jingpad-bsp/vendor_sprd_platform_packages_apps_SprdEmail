...
public abstract class EmailServiceStub extends IEmailService.Stub implements IEmailService {
    ...
    public void loadAttachment(final IEmailServiceCallback cb, final long accountId,
        ...
        try {
            ...
            /* SPRD: Modify for bug 572445 {@ */
            File downloadFile = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS +
                    "/" + attachment.mFileName);
            final boolean isFileNotExists = downloadFile == null || !downloadFile.exists();
            /* @} */

            //SPRD: Bug 499427 DUT fail to open attached audio file in email and shows error message
            if (mailbox.mType == Mailbox.TYPE_OUTBOX || mailbox.mType == Mailbox.TYPE_DRAFTS || mailbox.mType == Mailbox.TYPE_SENT) {
                ...
                if (TextUtils.isEmpty(message.mServerId)) {
                    if (isFileNotExists) {
                        copyAttachmentFromInternalToExternal(mContext, attachment);
                    }
                    cb.loadAttachmentStatus(messageId, attachmentId,
            ...
            /* SPRD: Modify for bug 572445 {@ */
            if (mailbox.mType == Mailbox.TYPE_SENT && remoteStore instanceof ImapStore) {
    ...
    /* SPRD: Modify for bug 572445 {@ */
    public boolean copyAttachmentFromInternalToExternal(Context context, Attachment attachment) {
        try {
            if (attachment == null || TextUtils.isEmpty(attachment.getCachedFileUri())) {
                return false;
            }
            InputStream in = context.getContentResolver().openInputStream(Uri.parse(attachment.getCachedFileUri()));
            attachment.mUiDestination = UIProvider.AttachmentDestination.EXTERNAL;
            String resultUri = AttachmentUtilities.saveAttachment(context, in, attachment, true);
     ...
     */
    /* UNISOC: Modify for bug1221819 @{ */
    public static class MessageRetrievalListenerBridge implements MessageRetrievalListener {
