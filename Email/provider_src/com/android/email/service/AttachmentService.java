...
public class AttachmentService extends Service implements Runnable {
        ...
        public boolean addRequest(final DownloadRequest request)
            ...
            synchronized (mLock) {
                ...
                if (!exists) {
                    ...
                    debugTrace("DownloadRequest #%d was already in the queue", requestId);
                }
            }
            return true;
        }
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (service != null) {
                        ...
                        if (service.mStop) {
                            LogUtils.d(LOG_TAG, "our service instance is gone, just leave");
    ...
    class ServiceCallback extends IEmailServiceCallback.Stub {
        ...
        public void loadAttachmentStatus(final long messageId, final long attachmentId,
                final int statusCode, final int progress) {
            debugTrace("ServiceCallback for attachment #%d", attachmentId);
    ...
    public int onStartCommand(final Intent intent, final int flags, final int startId) {
        LogUtils.i(LOG_TAG, "onStartCommand");  // UNISOC: Modify for bug1202804
        if (sRunningService == null) {
            sRunningService = this;
        }
        /* UNISOC: Modify for bug1202804 @{ */
    ...
    public void onCreate() {
        LogUtils.i(LOG_TAG, "Attachment Service onCreate");
        isRunning = true;
    }
    ...
    public void onDestroy() {
        LogUtils.i(LOG_TAG, "onDestroy"); // UNISOC: Modify for bug1202804
    ...
    public void run() {
        ...
        while (!mStop) {
            ...
            if (ecm != null) {
                /* UNISOC: Modify for bug1256156 {@ */
                try {
                    ecm.waitForConnectivity();
                } catch (IllegalStateException e) {
                    LogUtils.d(LOG_TAG, "EmailConnectivityManager has been unregistered stop now");
                    break;
                }
                /* @} */
            }
            ...
            if (sAttachmentChangedQueue.isEmpty()) {
                LogUtils.i(LOG_TAG, "Run() idle, wait for mLock (something to do)");
                synchronized (mLock) {
                    try {
                ...
                LogUtils.i(LOG_TAG, "sAttachmentChangedQueue is not Empty continue process");
            }
        }
    ...
    public synchronized void onChange(final Context context, final Attachment att) {
        ...
        if (priority == PRIORITY_NONE) {
            LogUtils.i(LOG_TAG, "Attachment #%d has no priority and will not be downloaded",
                    att.mId);
            ...
            if (req == null) {
                LogUtils.i(LOG_TAG, "Attachment #%d is a new download request", att.mId);
                 ...
                 *@{
                 */
                final AttachmentInfo attachInfo;
                if (AttachmentInfo.ENABLE_SPRD_DENY_PROCESS) {
                    attachInfo = new AttachmentInfo(context, att.mId, att.mSize, att.mFileName,
                            att.mMimeType, att.mAccountKey, att.mFlags, true);
                } else {
                    attachInfo = new AttachmentInfo(context, att);
                }
                /*@}*/
        ...
        if ((att.mFlags & Attachment.FLAG_DOWNLOAD_CANCELLED) != 0
                && mDownloadsInProgress.containsKey(att.mId)) {
            if (req == null) {
                req = new DownloadRequest(context, att);
            }
            cancelDownloadWithFlag(req);
        }
        /* @} */
        // Process the queue if we're in a wait
        kick();
    ...
    void markAttachmentAsFailed(final Attachment att) {
        ...
        att.mFlags &= ~flags;
        if (AttachmentInfo.ENABLE_SPRD_DENY_PROCESS) {
            final AttachmentInfo info = new AttachmentInfo(this, att.mId, att.mSize, att.mFileName,
                    att.mMimeType, att.mAccountKey, att.mFlags, true);
            if (info.mDenyFlags != AttachmentInfo.ALLOW) {
                // If mDenyFlags is not 0, it will transfer from AttachmentInfo.DENY_XXX to
                // MessageAttachmentBar.FLAG_DENY_XXX, so it need left shift 20bits. Such as, if mDenyFlags
                // is DENY_MALWARE (1<<0) of AttachmentInfo, it will saved to AttachmentColumns.FLAGS
                // at bit 20, and then MessageAttachmentBar get deny value like 'flags & FLAG_DENY_WALWARE'.
                att.mFlags |= (info.mDenyFlags<<20);
                LogUtils.i(LOG_TAG, "[AttachmentService][markAttachmentAsFailed]att.mFileName:%s, att.mFlags:%x",
                        att.mFileName, att.mFlags);
            }
        }
        cv.put(AttachmentColumns.FLAGS, att.mFlags);
        /*@}*/
    ...
    void markAttachmentAsCompleted(final Attachment att) {
        LogUtils.i(LOG_TAG, "markAttachmentAsCompleted  attachmentId  :" + att.mId);
        final ContentValues cv = new ContentValues();
        final int flags = Attachment.FLAG_DOWNLOAD_FORWARD | Attachment.FLAG_DOWNLOAD_USER_REQUEST;
        cv.put(AttachmentColumns.FLAGS, att.mFlags &= ~flags);
    ...
    synchronized void processQueue() {
        ...
        while (change != null) {
            ...
            if (attachment == null) {
                ...
                //sprd Modify bug495521 add for break endless loop
        ...
        /* SPRD modify for bug736423 @{ */
        final DownloadQueue cacheDownloadQueue = new DownloadQueue();
        /* @} */
        ...
        while (mDownloadsInProgress.size() < MAX_SIMULTANEOUS_DOWNLOADS) {
            ...
            if (getDownloadsForAccount(req.mAccountId) >= MAX_SIMULTANEOUS_DOWNLOADS_PER_ACCOUNT) {
                /* SPRD: Modify for bug736423 @{ */
                LogUtils.w(LOG_TAG, "Skipping #%d; maxed for acct %d, add reAdd to cacheDownloadQueue, wait for next processQueue",
                        req.mAttachmentId, req.mAccountId);
                cacheDownloadQueue.addRequest(req);
                /* @} */
                continue;
            }
            ...
            if (!req.mInProgress) {
                ...
                LogUtils.d(LOG_TAG, "we should return for dont download other att when start download for #%d", req.mAttachmentId);
        ...
        /* SPRD: Modify for bug736423 @{ */
        while (cacheDownloadQueue.getSize() > 0) {
            DownloadRequest req = cacheDownloadQueue.getNextRequest();
            if (req != null && !mDownloadsInProgress.containsKey(req.mAttachmentId)) {
                LogUtils.w(LOG_TAG, "reAdd #%d to mDownloadQueue", req.mAttachmentId);
                mDownloadQueue.addRequest(req);
            }
        }
        /* @} */
        ...
        try {
            while (c.moveToNext()) {
                ...
                if (account == null) {
                    ...
                    if (AttachmentInfo.ENABLE_SPRD_DENY_PROCESS) {
                        info = new AttachmentInfo(this, att.mId, att.mSize, att.mFileName,
                                att.mMimeType, att.mAccountKey, att.mFlags, true);
                    } else {
                        info = new AttachmentInfo(this, att);
                    }
                    /*@}*/
                    if (info.isEligibleForDownload()) {
                        ...
                        if (att.mContentId != null || canPrefetchForAccount(account, cacheDir)) {
                            ...
                            LogUtils.d(LOG_TAG, "start auto download #%d ", att.mId);
                            tryStartDownload(req);
                            break;
                        }
                    } else {
    ...
    /* UNISOC: Modify for bug1047400 {@ */
    synchronized void cancelDownloadWithFlag(final DownloadRequest req) {
        LogUtils.i(LOG_TAG, "Cancelling download with flag for Attachment #%d", req.mAttachmentId);
    ...
    synchronized void cancelDownload(final DownloadRequest req) {
        LogUtils.i(LOG_TAG, "Cancelling download for Attachment #%d", req.mAttachmentId);
        req.mInProgress = false;
        mDownloadsInProgress.remove(req.mAttachmentId);
        // Remove the download from our queue, and then decide whether or not to add it back.
        mDownloadQueue.removeRequest(req);
        ...
        if (req.mRetryCount > CONNECTION_ERROR_MAX_RETRIES) {
            ...
            LogUtils.w(LOG_TAG, "Moving to end of queue, will retry " + req.mAttachmentId);
    ...
    synchronized void endDownload(final long attachmentId, final int statusCode) {
        LogUtils.i(LOG_TAG, "Finishing download #%d, #%d", attachmentId, statusCode);
        ...
        if (statusCode == EmailServiceStatus.CONNECTION_ERROR) {
            ...
            if (req != null) {
                ...
                LogUtils.w(LOG_TAG, "Attachment is not in request queue");
            }
    ...
    synchronized int getDownloadsForAccount(final long accountId) {
        ...
        for (final DownloadRequest req: mDownloadsInProgress.values()) {
            if (req.mAccountId == accountId) {
                LogUtils.d(LOG_TAG, "getDownloadsForAccount req.mAttachmentId #%d ", req.mAttachmentId);
