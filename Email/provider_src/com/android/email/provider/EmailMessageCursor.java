                    ...
                    if (TextUtils.isEmpty(sanitizedHtml)) {
                        ...
                        try {
                            bodyCursor = cr.query(
                                    ...
                                    new String[] {
                                        EmailContent.BodyColumns.TEXT_CONTENT
                                    },
                                    ...
                                    null);
                            while (bodyCursor != null && bodyCursor.moveToNext()) {
                                String textContent = bodyCursor.getString(0);
                                if (!TextUtils.isEmpty(textContent)) {
                                    mTextParts.put(position, textContent);
                                }
                                LogUtils.v(LogUtils.TAG, "Did not find html body for messageId " + messageId + ", Try to find body text from Database");
                            }
                        } finally {
                            if (bodyCursor != null) {
                                bodyCursor.close();
