...
public class EmailProvider extends ContentProvider
    ...
    private static final int UI_LOCAL_SEARCH = UI_BASE + 23;  //SPRD:bug475886 add local search function
    ...
    private static final int VIPMEMBER_BASE = 0xC000;
    ...
    private void fixParentKeys(SQLiteDatabase db) {
        ...
        try {
            ...
            LogUtils.e(TAG, e, "Database or disk is full.");
        }
        /* @} */
    }
    ...
    public SQLiteDatabase getDatabase(Context context) {
        synchronized (sDatabaseLock) {
            ...
            /* UNISOC: Modify for bug1179193 @{ */
            mDatabase = new DBHelper.DatabaseHelper(context, DATABASE_NAME).getWritableDatabase();
            mBodyDatabase = new DBHelper.BodyDatabaseHelper(context, BODY_DATABASE_NAME).getWritableDatabase();
            /* @} */
            ...
            switch (match) {
                case BODY:
                    ...
                    if(Utils.SUPPORT_LOCAL_SEARCH) {
                        final String htmlContent = values.getAsString(BodyColumns.HTML_CONTENT);
                        final String textContent = values.getAsString(BodyColumns.TEXT_CONTENT);
                        dbValues.remove(BodyColumns.HTML_CONTENT);
                        if (!TextUtils.isEmpty(htmlContent) && TextUtils.isEmpty(textContent)) {
                            dbValues.put(BodyColumns.TEXT_CONTENT, TextUtilities.convertFromHtmlToText(htmlContent));
                        }
                    } else {
                ...
                case MESSAGE:
                    ...
                    if (values.containsKey(MessageColumns.SUBJECT)) {
                        String subject = values.getAsString(MessageColumns.SUBJECT);
                        if (!TextUtils.isEmpty(subject)) {
                            values.put(MessageColumns.SUBJECT_SORT_KEY,
                                    Message.getPinYinSortKey(subject).toLowerCase());
                        }
                    }

                    if (values.containsKey(MessageColumns.DISPLAY_NAME)) {
                        String displayName = values.getAsString(MessageColumns.DISPLAY_NAME);
                        if (!TextUtils.isEmpty(displayName)) {
                            values.put(MessageColumns.DISPLAY_NAME_SORT_KEY,
                                    Message.getPinYinSortKey(displayName).toLowerCase());
                        }
                    }
                case ATTACHMENT:
                case MAILBOX:
    ...
    public boolean onCreate() {
        ...
        final Intent updateAllWidgetsIntent =
                new Intent(Utils.ACTION_NOTIFY_DATASET_CHANGED);
        updateAllWidgetsIntent.setClassName(context.getPackageName(),
                com.android.email.provider.WidgetProvider.class.getName());
    ...
    private static void init(final Context context) {
        ...
        synchronized (sURIMatcher) {
            ...
            /* SPRD:bug475886 add local search function @{ */
            sURIMatcher.addURI(EmailContent.AUTHORITY, "uiLocalSearch/#", UI_LOCAL_SEARCH);
            /* @} */
            ...
            switch (match) {
                ...
                case BODY_ID: {
                    final ProjectionMap map = new ProjectionMap.Builder()
                            ...
                            .build();
                    /* SPRD:bug475886 add local search function @{ */
                    if (Utils.SUPPORT_LOCAL_SEARCH && map.containsKey(BodyColumns.HTML_CONTENT)) {
                        throw new IllegalArgumentException(
                                "Body content cannot be returned in the cursor");
                    /* UNISOC: Modify for bug1010702 {@ */
                    } else if (!Utils.SUPPORT_LOCAL_SEARCH && (map.containsKey(BodyColumns.HTML_CONTENT) ||
    ...
    /* SPRD modify for bug652656 {@ */
    private static final String SET_ATTACHMENT_INVALID_TO_DOWNLOAD_BY_MESSAGE_KEY = "UPDATE Attachment SET "
            + AttachmentColumns.FLAGS
            + "="
            + AttachmentColumns.FLAGS
            + "|"
            + Attachment.FLAG_INVALID_TO_DOWNLOAD + "  WHERE "
            + AttachmentColumns.MESSAGE_KEY + "=";

    private static final String SET_ATTACHMENT_VALID_TO_DOWNLOAD_BY_MESSAGE_KEY = "UPDATE Attachment SET "
            + AttachmentColumns.FLAGS
            + "="
            + AttachmentColumns.FLAGS
            + "&~"
            ...
            switch (match) {
                ...
                case VIPMEMBER_ID:
                    ...
                    if (match == SYNCED_MESSAGE_ID) {
                        ...
                        long mailboxId = -1;
                        long accountId = -1;
                        final Cursor c = db.rawQuery(GET_MESSAGE_DETAILS, new String[] {
                                id
                        });
                            ...
                            try {
                                if (c.moveToFirst()) {
                                    ...
                                    isEas = context.getString(R.string.protocol_eas)
                                            .equals(protocol);
                                    isPop3 = context.getString(R.string.protocol_pop3)
                                            .equals(protocol);
                        ...
                        if (isPop3 && values.containsKey(MessageColumns.MAILBOX_KEY)) {
                            final long mailBoxId = values.getAsLong(MessageColumns.MAILBOX_KEY);
                            Mailbox mailbox = Mailbox.restoreMailboxWithId(context, mailBoxId);
                            if (mailbox != null) {
                                final String updateSql = (mailbox.mType == Mailbox.TYPE_TRASH) ? SET_ATTACHMENT_INVALID_TO_DOWNLOAD_BY_MESSAGE_KEY
                                        : SET_ATTACHMENT_VALID_TO_DOWNLOAD_BY_MESSAGE_KEY;
                                try {
                                    db.execSQL(updateSql + id);
                                } catch (SQLException e) {
                                    LogUtils.e(TAG, e, "Error while update attachment flags updateSql: "
                                            + updateSql);
                                }
                            }
                        }
                    ...
                    /* SPRD: Modify for bug 567808 {@ */
                    long mFolderId = -1;
                    long mAccountId = -1;

                    final Cursor mCursor = query(Message.CONTENT_URI.buildUpon()
                    ...
                    if (mCursor != null) {
                        try {
                            if (mCursor.moveToFirst()) {
                                mFolderId = mCursor
                                        .getLong(MESSAGE_KEYS_MAILBOX_KEY_COLUMN);
                                mAccountId = mCursor
                                        .getLong(MESSAGE_KEYS_ACCOUNT_KEY_COLUMN);
                            }
                        } finally {
                            mCursor.close();
                        }
                    ...
                    /* @} */
                    if (mFolderId != -1 && mAccountId != -1) {
                        notifyUIFolder(mFolderId, mAccountId);
                    }
                    /* @} */
    ...
    private static void writeBodyFile(final Context c, final long messageId, final String ext,
        ...
        if (TextUtils.isEmpty(content)) {
            ...
            if (Utils.SUPPORT_SUPER_LINK && ext.equals("html")) {
                content = Utils.getHtmlContext(content);
            }
            try {
                w.write(content);
            } finally {
    ...
    public ContentProviderResult[] applyBatch(ArrayList<ContentProviderOperation> operations)
        ...
        try {
            ...
            /* SPRD: Modify for bug604947 {@ */
            try {
                db.endTransaction();
            } catch (SQLiteException e) {
                LogUtils.e(TAG, "endTransaction occur SQLiteException: %s", e.getMessage());
            } finally {
     ...
     *@{
     */
    private static final String CONVERSATION_MAIL_PRIORITY =
             "CASE WHEN (" + MessageColumns.FLAGS + "&" + Message.FLAG_TYPE_PRIORITY_HIGH +
                 ") !=0 THEN " + UIProvider.MailPriority.HIGH +
             " WHEN (" + MessageColumns.FLAGS + "&" + Message.FLAG_TYPE_PRIORITY_LOW +
                 ") !=0 THEN " + UIProvider.MailPriority.LOW +
                " ELSE " + UIProvider.MailPriority.NORMAL + " END";
    /*@}*/
    ...
    private static ProjectionMap getMessageListMap() {
        if (sMessageListMap == null) {
            sMessageListMap = ProjectionMap.builder()
                 ...
                 *@{
                 */
                .add(UIProvider.ConversationColumns.MAIL_PRIORITY,
                        CONVERSATION_MAIL_PRIORITY)
                /*@}*/
                .build();
        }
    ...
    private static ProjectionMap getMessageViewMap() {
        if (sMessageViewMap == null) {
            sMessageViewMap = ProjectionMap.builder()
                 ...
                 *@{
                 */
                .add(UIProvider.MessageColumns.MAIL_PRIORITY, CONVERSATION_MAIL_PRIORITY)
                /*@}*/
                .build();
        }
    ...
    /* UNISOC: Modify for bug1235324 @{ */
    private synchronized static ProjectionMap getFolderListMap() {
        if (sFolderListMap == null) {
            sFolderListMap = ProjectionMap.builder()
                ...
                .add(UIProvider.FolderColumns.LOCAL_SEARCH_URI, uriWithId("uiLocalSearch"))
                .build();
        }
        return sFolderListMap;
    }
    private static ProjectionMap sFolderListMap = null;
    /* @} */
     ...
     */
    /* UNISOC: Modify for bug1235324 @{ */
    private synchronized static ProjectionMap getAccountListMap(Context context) {
    ...
    private static ProjectionMap sAccountListMap = null;
    /* @} */

    /* UNISOC: Modify for bug1235324 @{ */
    private synchronized static ProjectionMap getQuickResponseMap() {
    ...
    private static ProjectionMap sQuickResponseMap = null;
    /* @} */
    ...
    private static StringBuilder genSelect(ProjectionMap map, String[] projection,
        ...
        if (projection == null) {
            LogUtils.w("EmailProvider", "genSelect->projection == null ");
            return sb;
        }
        /* @}  */
    ...
    private MessageQuery genQueryViewMessage(String[] uiProjection, String id) {
        ...
        if (msg != null) {
            ...
            /* SPRD: Modify for bug597947 {@ */
            if (body != null && body.mSourceKey != 0) {
                final String refMessageId = EmailContent.CONTENT_URI + "/uimessage/" + body.mSourceKey;
                LogUtils.i(LogUtils.TAG, "[EmailProvider][genQueryViewMessage]REF_MESSAGE_ID:%s", refMessageId);
                values.put(UIProvider.MessageColumns.REF_MESSAGE_ID, refMessageId);
            }
            /* @} */
...
//                    }
                    ...
                    uiAtt.state = att.mUiState; //UNISOC: Modify for bug 1168336
                    uiAtt.uri = uiUri("uiattachment", att.mId);
                    /* SPRD: Modify for bug502703 {@ */
            ...
            if (UIProvider.ENABLE_MAIL_PRIORITY_FEATURE) {
                if ((msg.mFlags & Message.FLAG_TYPE_PRIORITY_HIGH) != 0) {
                    values.put(UIProvider.MessageColumns.MAIL_PRIORITY, UIProvider.MailPriority.HIGH);
                    LogUtils.i(LogUtils.TAG, "[EmailProvider][genQueryViewMessage]MailPriority.HIGH");
                } else if ((msg.mFlags & Message.FLAG_TYPE_PRIORITY_LOW) != 0) {
                    values.put(UIProvider.MessageColumns.MAIL_PRIORITY, UIProvider.MailPriority.LOW);
                    LogUtils.i(LogUtils.TAG, "[EmailProvider][genQueryViewMessage]MailPriority.LOW");
                } else {
                    values.put(UIProvider.MessageColumns.MAIL_PRIORITY, UIProvider.MailPriority.NORMAL);
                }
            }
            /*@}*/
        }
    ...
    private static String getQueryMailboxMessagesOrderBy(Uri uri) {
        String sortOrder = uri
                .getQueryParameter(UIProvider.ConversationSortOrder.SORT_ORDER_PARAMETER);
        int sortOrderFlag = 0;
        if (!TextUtils.isEmpty(sortOrder)) {
            try {
                sortOrderFlag = Integer.parseInt(sortOrder);
            } catch (NumberFormatException e) {
                LogUtils.w(TAG, "getQueryMailboxMessagesOrderBy Unknown sortOrder" + sortOrder);
            }
        }

        StringBuilder sb = new StringBuilder(" ORDER BY ");
        int orderMethod = UIProvider.ConversationSortOrder.getOrderMethod(sortOrderFlag);
        int asendence = UIProvider.ConversationSortOrder.getAsc(sortOrderFlag);

        switch (orderMethod) {
            case UIProvider.ConversationSortOrder.DISPLAY_NAME:
                sb.append(MessageColumns.DISPLAY_NAME_SORT_KEY);
                break;
            case UIProvider.ConversationSortOrder.SUBJECT:
                sb.append(MessageColumns.SUBJECT_SORT_KEY);
                break;
            default:
                sb.append(MessageColumns.TIMESTAMP);
                break;
        }

        sb.append(' ').append((asendence == UIProvider.ConversationSortOrder.ASC) ? "ASC" : "DESC");
        return sb.toString();
    }
     ...
     * @param unseenOnly <code>true</code> to only return unseen messages
     * @param selection statement
     * @return the SQLite query to be executed on the EmailProvider database
     */
    private static Cursor getVirtualMailboxMessagesCursor(SQLiteDatabase db, String[] uiProjection,
            long mailboxId, final boolean unseenOnly, String selection, String orderBy) {
        ContentValues values = new ContentValues();
        ...
        switch (mailboxType) {
            ...
            case Mailbox.TYPE_VIP:
                /* SPRD: Modify for bug712883 @{ */
                sb.append(MessageColumns.MAILBOX_KEY +
                        " NOT IN (SELECT " + MailboxColumns.ID
                        + " FROM " + Mailbox.TABLE_NAME + " WHERE " + MailboxColumns.TYPE + "="
                        + Mailbox.TYPE_TRASH + " OR " + MailboxColumns.TYPE + " = "
                        + Mailbox.TYPE_SEARCH + " OR " + MailboxColumns.TYPE + " = "
                        + Mailbox.TYPE_DRAFTS + " OR " + MailboxColumns.TYPE + " = "
                        + Mailbox.TYPE_OUTBOX + " OR " + MailboxColumns.TYPE + " = "
                        + Mailbox.TYPE_SENT + ")");
                /* @} */
                break;
            default:
    ...
    private static int getCapabilities(Context context, final Account account) {
        ...
        /* SPRD:bug475886 add local search function @{ */
        if (Utils.SUPPORT_LOCAL_SEARCH) {
            capabilities |= AccountCapabilities.LOCAL_SEARCH;
        }
        /* @} */
    ...
    private String genQueryAccount(String[] uiProjection, String id) {
        ...
        if (projectionColumns.contains(UIProvider.AccountColumns.SettingsColumns.SNIPPET_SHOW_LINES)) {
            values.put(UIProvider.AccountColumns.SettingsColumns.SNIPPET_SHOW_LINES,
                    mailPrefs.getSnippetShowLines());
        }
    ...
    private void addCombinedAccountRow(MatrixCursor mc) {
        ...
        if (colPosMap.containsKey(UIProvider.AccountColumns.UNDO_URI)) {
            // SPRD: Modify bug492833, parameter setting error.
        ...
        /* @} */
        if (colPosMap.containsKey(UIProvider.AccountColumns.SettingsColumns.SNIPPET_SHOW_LINES)) {
            values[colPosMap.get(UIProvider.AccountColumns.SettingsColumns.SNIPPET_SHOW_LINES)] =
                    mailPrefs.getSnippetShowLines();
        }
    ...
    private Object[] getVirtualMailboxRow(long accountId, int mailboxType, String[] projection) {
        ...
        for (int i = 0; i < projection.length; i++) {
            ...
            if (column.equals(UIProvider.FolderColumns._ID)) {
                ...
                if (mailboxType == Mailbox.TYPE_INBOX && accountId == COMBINED_ACCOUNT_ID) {
                    ...
                    final int starredCount = EmailContent.count(getContext(), Message.CONTENT_URI,
                            accountKeyClause + MessageColumns.FLAG_FAVORITE + "=1 AND "+MessageColumns.MAILBOX_KEY +
                            " NOT IN (SELECT " + MailboxColumns.ID + " FROM " + Mailbox.TABLE_NAME +
                            " WHERE " + MailboxColumns.TYPE + "=" + Mailbox.TYPE_TRASH + ")", whereArgs);
                    /* @} */
                    values[i] = starredCount;
                } else if (mailboxType == Mailbox.TYPE_VIP) {
                    /* SPRD: Modify for bug709933 @{ */
                    values[i] = VipMemberCache.getVipMessagesCount(getContext(), accountId, true);;
                    /* @} */
                }
            ...
            } else if (column.equals(UIProvider.FolderColumns.LOCAL_SEARCH_URI)) {
                values[i] = combinedUriString("uiLocalSearch", idString);
            /* SPRD:bug628647 add for stared email crash issue @{ */
    ...
    private Cursor uiFolders(final Uri uri, final String[] uiProjection) {
        ...
        if (id.equals(COMBINED_ACCOUNT_ID_STRING)) {
            ...
            if (c.getCount() > 0) {
                ...
                vc.close();// UNISOC: Modify for bug1179193
                return c;
            }
        }
    }
    ...
    private Cursor uiQuery(int match, Uri uri, String[] uiProjection, final boolean unseenOnly) {
        ...
        switch(match) {
            ...
            case UI_MESSAGES:
                ...
                String orderByClause = getQueryMailboxMessagesOrderBy(uri);
                /* SPRD:bug475886 add local search function @{ */
                if (isVirtualMailbox(mailboxId)) {
                    ...
                    c = db.rawQuery(
                            genQueryMailboxMessages(uiProjection, unseenOnly, null, orderByClause),
                            new String[] {
                                id
                            });
                }
                /* @} */
                if (getVirtualMailboxType(mailboxId) == Mailbox.TYPE_VIP) {
                    notifyUri = VipMember.UIPROVIDER_VIPMEMBER_NOTIFIER;
                } else {
            ...
            /* SPRD:bug475886 add local search function @{ */
            case UI_LOCAL_SEARCH:
                c = uiLocalSearch(uri, uiProjection, unseenOnly);
                notifyUri = UIPROVIDER_CONVERSATION_NOTIFIER.buildUpon().appendPath(id).build();
                break;
            /* @} */
        }
    ...
    private Uri uiSaveMessage(Message msg, Mailbox mailbox, Bundle extras) {
         ...
         */
        if (UIProvider.ENABLE_MAIL_PRIORITY_FEATURE) {
            final int mailPriority = extras.containsKey(UIProvider.MessageColumns.MAIL_PRIORITY) ? extras
                    .getInt(UIProvider.MessageColumns.MAIL_PRIORITY) : UIProvider.MailPriority.NORMAL;
            if (mailPriority == UIProvider.MailPriority.HIGH) {
                flags |= Message.FLAG_TYPE_PRIORITY_HIGH;
                LogUtils.i(LogUtils.TAG, "[EmailProvider][uiSaveMessage]MailPriority.HIGH");
            } else if (mailPriority == UIProvider.MailPriority.LOW){
                flags |= Message.FLAG_TYPE_PRIORITY_LOW;
                LogUtils.i(LogUtils.TAG, "[EmailProvider][uiSaveMessage]MailPriority.LOW");
            }
        }
        /*@}*/
        ...
        /* UNISOC: Modify for bug1178717 {@ */
        if (!atts.isEmpty() && hasUnloadedAttachments) {
            Utility.showToast(context, R.string.message_view_attachment_background_load);
        }
        /* @} */

        /* SPRD:bug609773 bug620254 bug650105 modify begin @{ */
    ...
    private Uri uiSaveDraftMessage(final long accountId, final Bundle extras) {
        ...
        /* SPRD: Modify for 497553 {@ */
        Uri uri = uiSaveMessage(msg, mailbox, extras);
        final Mailbox draftMailBox = Mailbox.restoreMailboxWithId(getContext(), msg.mMailboxKey);
        if (draftMailBox != null) {
            notifyUIFolder(draftMailBox.mId, draftMailBox.mAccountKey);
        }
        return uri;
        /* @} */
    }
    ...
    private int uiUpdateAttachment(Uri uri, ContentValues uiValues) {
        ...
        if (stateValue != null) {
            ...
            /* SPRD modify for bug695420, bug732738, bug734773 {@ */
            final ContentValues values = new ContentValues();
            if (state == UIProvider.AttachmentState.NOT_SAVED) {
                ...
                /* UNISOC: Modify for bug1047400 {@ */
                attachment.mFlags |= Attachment.FLAG_DOWNLOAD_CANCELLED;
                /* @} */
                ...
                result = 1;
            } else if (state == UIProvider.AttachmentState.REDOWNLOADING) {
                values.put(AttachmentColumns.FLAGS,
                        attachment.mFlags &= ~Attachment.FLAG_DOWNLOAD_USER_REQUEST);
                values.put(AttachmentColumns.UI_STATE, UIProvider.AttachmentState.NOT_SAVED);
                ...
                LogUtils.i(TAG, "REDOWNLOADING Cancle current request id:" + attachmentId);
                mBackgroundHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        values.put(AttachmentColumns.UI_STATE,
                                UIProvider.AttachmentState.DOWNLOADING);
                        Integer destinationValue = uiValues
                                .getAsInteger(UIProvider.AttachmentColumns.DESTINATION);
                        values.put(AttachmentColumns.UI_DESTINATION,
                                destinationValue == null ? Integer.valueOf(0) : destinationValue);  //UNISOC: Modify dor bug1234478
                        values.put(AttachmentColumns.FLAGS,
                                attachment.mFlags | Attachment.FLAG_DOWNLOAD_USER_REQUEST);

                        if (values.containsKey(AttachmentColumns.LOCATION) && TextUtils
                                .isEmpty(values.getAsString(AttachmentColumns.LOCATION))) {
                            LogUtils.w(TAG, new Throwable(), "attachment with blank location");
                        }
                        attachment.update(context, values);

                        LogUtils.i(TAG, "REDOWNLOADING issue new request");
                    }
                }, 500);
                result = 1;
            } else if (state == UIProvider.AttachmentState.DOWNLOADING) {
                ...
                attachment.update(context, values);
                result = 1;
            } else if (state == UIProvider.AttachmentState.SAVED) {
    ...
    private ContentValues convertUiMessageValues(Message message, ContentValues values) {
        ...
        for (String columnName : values.keySet()) {
            ...
            } else if (columnName.equals(MessageColumns.FLAGS)) {
                /* SPRD: Modify for bug517754 {@ */
                putIntegerLongOrBoolean(ourValues, MessageColumns.FLAGS, val);
                /* @} */
            }else {
     ...
     */
    /*SPRD: modify for bug611469 {@*/
    private void addToSequence(Uri uri, ContentProviderOperation op, Long messageId) {
    /* @}*/
        ...
        if (sequenceString != null) {
            ...
            /*SPRD: modify for bug611469 {@*/
            final MailPrefs mailPrefs = MailPrefs.get(getContext());
            final Set<String> undoMessage = Sets.newHashSet(mailPrefs.getUndoMessage());
            undoMessage.add(messageId + "");
            mailPrefs.setUndoMessage(undoMessage);
            /* @}*/
        }
    }
    ...
    private int uiUpdateMessage(Uri uri, ContentValues values, boolean forceSync) {
        ...
        if (values.containsKey(UIProvider.MessageOperations.RESPOND_COLUMN)) {
            ...
            try {
                /* SPRD modify for bug681671 {@ */
                if ((msg.mFlags & Message.FLAG_MEETTING_RESPONSED) != 0) {
                    return 0;
                }
                ...
                msg.mFlags = msg.mFlags | Message.FLAG_MEETTING_RESPONSED;
                msg.save(context);
    ...
    private void handleMessageUpdateNotifications(final Uri uri, final String messageId,
        ...
        /* SPRD modify for bug 640299 {@ */
        boolean isUiProvider = uri.getBooleanQueryParameter(IS_UIPROVIDER, false);
        if (!isUiProvider) {
            notifyUIConversation(uri);
        }
        ...
        if (values.containsKey(MessageColumns.FLAG_READ) ||
            ...
            if (c != null) {
                try {
                    if (c.moveToFirst()) {
                        long mailboxId = c.getLong(MESSAGE_KEYS_MAILBOX_KEY_COLUMN);
                        long accountId = c.getLong(MESSAGE_KEYS_ACCOUNT_KEY_COLUMN);

                        notifyUIFolder(mailboxId, accountId);
                        /* SPRD: Modify for bug 567808 {@ */
                        notifyUI(UIPROVIDER_RECENT_FOLDERS_NOTIFIER
                                .buildUpon()
                                .appendPath(Long.toString(accountId))
                                .build(), null);
                        /* @} */

                        Mailbox mailbox = Mailbox.restoreMailboxWithId(getContext(), mailboxId);
                        if (mailbox != null
                                && (isUiProvider || mailbox.mType != Mailbox.TYPE_INBOX)) {
    ...
    private int uiDeleteMessage(Uri uri) {
        ...
        if (account != null && Account.DELETE_POLICY_NEVER == account.getDeletePolicy()
            && mailbox.mType == Mailbox.TYPE_INBOX) {
            values.put(MessageColumns.FLAGS, msg.mFlags | Message.FLAG_LOCALLY_REMOVED);
        }
        /* @} */
    ...
    private void notifyUIFolder(final String folderId, final long accountId) {
        ...
        /* SPRD: Modify for 496153 {@ */
        notifyUI(UIPROVIDER_FOLDER_NOTIFIER,
                getVirtualMailboxId(accountId, Mailbox.TYPE_UNREAD));
        notifyUI(UIPROVIDER_FOLDER_NOTIFIER,
                getVirtualMailboxId(COMBINED_ACCOUNT_ID, Mailbox.TYPE_UNREAD));
        notifyUI(UIPROVIDER_CONVERSATION_NOTIFIER,
                getVirtualMailboxId(accountId, Mailbox.TYPE_UNREAD));
        notifyUI(UIPROVIDER_CONVERSATION_NOTIFIER,
                getVirtualMailboxId(COMBINED_ACCOUNT_ID, Mailbox.TYPE_UNREAD));
        ...
        /* SPRD: Modify for bug493891 {@ */
        notifyUI(UIPROVIDER_FOLDER_NOTIFIER,
                getVirtualMailboxId(accountId, Mailbox.TYPE_STARRED));
        notifyUI(UIPROVIDER_FOLDER_NOTIFIER,
                getVirtualMailboxId(COMBINED_ACCOUNT_ID, Mailbox.TYPE_STARRED));
        notifyUI(UIPROVIDER_CONVERSATION_NOTIFIER,
                getVirtualMailboxId(accountId, Mailbox.TYPE_STARRED));
        notifyUI(UIPROVIDER_CONVERSATION_NOTIFIER,
                getVirtualMailboxId(COMBINED_ACCOUNT_ID, Mailbox.TYPE_STARRED));
    ...
    private void notifiyVirtualMailbox(long accountId, int type) {
        ...
        if (type == Mailbox.TYPE_VIP) {
            /* SPRD: Modify for bug709933 @{ */
            notifyUI(VipMember.UIPROVIDER_VIPMEMBER_NOTIFIER, type);
            /* @} */
        } else {
            notifyUI(UIPROVIDER_CONVERSATION_NOTIFIER,
        ...
        if (mailbox != null) {
            RefreshStatusMonitor.getInstance(getContext())
                ...
                public void onRefreshCompleted(long mailboxId, int result) {
                    /* SPRD: Modify for bug670147 @{ */
                    new EmailAsyncTask<Void, Void, Mailbox>(null /* no cancel */) {
                        @Override
                        protected Mailbox doInBackground(Void... params) {
                        ...
                        @Override
                        protected void onSuccess(final Mailbox mailbox) {
                            notifyUIFolder(mailbox.mId, mailbox.mAccountKey);
                        }
    ...
    private Cursor uiSearch(Uri uri, String[] projection) {
        ...
        /* SPRD:bug475886 add local search function @{ */
        int approach = Integer.parseInt(uri.getQueryParameter(UIProvider.SearchQueryParameters.QUERY_APPROACH));
        if ((approach & SearchParams.SEARCH_BY_RECIVER) == 0
            ...
            throw new IllegalArgumentException("Invalid search approach: " + approach
                    + " in search query");
        }
        ...
        LogUtils.d(LogUtils.LOCAL_SEARCH_TAG, "uiSearch filter is " + filter + " approach is " + approach);
        mSearchParams = new SearchParams(inbox.mId, filter, searchMailboxId, approach);
        /* @} */
    ...
    /* SPRD:bug475886 add local search function @{ */
    private static String getEscapesdString(String sqlStr) {
        if (TextUtils.isEmpty(sqlStr)) {
            return sqlStr;
        }

        sqlStr = sqlStr.replaceAll("\\\\", "\\\\\\\\").replaceAll("%", "\\\\%")
                .replaceAll("_", "\\\\_").replaceAll("'", "''").trim();
        return sqlStr;
    }

    private Cursor uiLocalSearch(Uri uri, String[] projection, boolean unseenOnly) {

        String query = uri.getQueryParameter(UIProvider.SearchQueryParameters.QUERY);
        int approach = Integer.parseInt(uri
                .getQueryParameter(UIProvider.SearchQueryParameters.QUERY_APPROACH));
        String id = uri.getLastPathSegment();
        Long mailboxId = Long.parseLong(id);
        Context context = getContext();
        Folder folder = getFolder(context, mailboxId);
        SQLiteDatabase db = getDatabase(context);
        Uri notifyUri = null;

        if (folder == null) {
            LogUtils.e(LogUtils.LOCAL_SEARCH_TAG, "uiLocalSearch folder is null");
            return new MatrixCursor(projection);
        }

        if (TextUtils.isEmpty(query)) {
            LogUtils.e(LogUtils.LOCAL_SEARCH_TAG, "uiLocalSearch query is Empty");
            return new MatrixCursor(projection);
        }

        query = getEscapesdString(query);
        ...
        LogUtils.d(LogUtils.LOCAL_SEARCH_TAG, "uiLocalSearch query is " + query + " approach is "
                + approach);

        StringBuilder sb = new StringBuilder();

        if ((approach & SearchParams.SEARCH_BY_SENDER) != 0) {
            sb.append(Message.buildLocalSearchSelection(query, MessageColumns.FROM_LIST));
            sb.append(Message.buildLocalSearchSelection(query, MessageColumns.DISPLAY_NAME));
        }

        if ((approach & SearchParams.SEARCH_BY_RECIVER) != 0) {
            sb.append(Message.buildLocalSearchSelection(query, MessageColumns.TO_LIST));
            sb.append(Message.buildLocalSearchSelection(query, MessageColumns.CC_LIST));
            sb.append(Message.buildLocalSearchSelection(query, MessageColumns.BCC_LIST));
        }

        if ((approach & SearchParams.SEARCH_BY_SUBJECT) != 0) {
            sb.append(Message.buildLocalSearchSelection(query, MessageColumns.SUBJECT));
        }

        if ((approach & SearchParams.SEARCH_BY_CONTENT) != 0) {
            sb.append(Message.buildLocalSearchBodySelection(query));
        }

        // The return value of buildLocalSearchSelection is endwith or, so the last or is
        // unnecessary so make it " or 0"
        sb.append(0);

        Cursor c;
        String selection = sb.toString();
        String orderBy = getQueryMailboxMessagesOrderBy(uri);
        if (isVirtualMailbox(mailboxId)) {
            c = getVirtualMailboxMessagesCursor(db, projection, mailboxId, unseenOnly,
                    selection, orderBy);
        } else {
            c = db.rawQuery(genQueryMailboxMessages(projection, unseenOnly, selection, orderBy),
                    new String[] {
                        id
                    });
        }
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (PreferenceKeys.REMOVAL_ACTION.equals(key) ||
                ...
                PreferenceKeys.SNIPPET_SHOW_LINES.equals(key)) {
            notifyUI(UIPROVIDER_ALL_ACCOUNTS_NOTIFIER, null);
        }
    }
    ...
    public static boolean setServicesEnabledSync(Context context) {
        ...
        try {
            ...
            LogUtils.e(TAG, "setServicesEnabledSync occur SQLiteException: %s",
                    e.getMessage());
            setServicesEnabled(context, false);
            return false;
        } finally {
            if (c != null) {
                c.close();
