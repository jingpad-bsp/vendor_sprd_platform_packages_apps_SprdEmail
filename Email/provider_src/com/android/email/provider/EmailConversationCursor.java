...
public class EmailConversationCursor extends CursorWrapper implements
    ...
    public EmailConversationCursor(final Context context, final Cursor cursor,
        ...
        if (mailbox != null) {
            ...
            if (mailbox.mUiSyncStatus == EmailContent.SYNC_STATUS_BACKGROUND
                 ...
                 * The code just let UI display loading
                 * because of (system.currentTimeMillis() -
                 * mailbox.mSyncTime > AUTO_REFRESH_INTERVAL_MS and other
                 * conditions) but maybe it do not do sync really in fact.
                 * Thus it cause loading state can not be update and always
                 * display loading in UI in the case. @{
                 */
                 /*if (mailbox.mSyncInterval == 0
                         && (Mailbox.isSyncableType(mailbox.mType)
                                || mailbox.mType == Mailbox.TYPE_SEARCH)
                         /// If the mUiLastSyncResult is STORAGE_ERROR, should reset the
                         //   SyncStatus from LOADING TO COMPLETE to avoid refresh always.
                         && mailbox.mUiLastSyncResult != UIProvider.LastSyncResult.STORAGE_ERROR
                    ...
                    if (mailbox != null) {
                        ...
                        if (mailbox.mSyncInterval == 0) {
                            ...
                            if (timeSinceLastSync > AUTO_REFRESH_INTERVAL_MS) {
                                ...
                                /* UNISOC: Modify for bug1179191 @{ */
                                Cursor c = resolver.query(refreshUri, null, null, null, null);
                                if (null != c) {
                                    c.close();
                                }
                                /* @} */
    ...
    private ConversationInfo generateConversationInfo() {
        ...
        if (mMailboxTypeId == Mailbox.TYPE_SENT || mMailboxTypeId == Mailbox.TYPE_DRAFTS
