...
public final class DBHelper {
    ...
    //              version above 12.0
    // Version 128: Added VIP table
    // Version 129: Add sort_key for pinyin sort
    ...
    static void createVipMemberTable(SQLiteDatabase db) {
        String s = " (" + EmailContent.RECORD_ID + " integer primary key autoincrement, "
                + VipMember.ACCOUNT_KEY + " integer, "
                + VipMember.EMAIL_ADDRESS + " text, "
                + VipMember.DISPLAY_NAME + " text" + ");";
        db.execSQL("create table if not exists " + VipMember.TABLE_NAME + s);
    }
    ...
    static void createMessageTable(Context context, SQLiteDatabase db) {
        String messageColumns = MessageColumns.DISPLAY_NAME + " text, "
            ...
            + MessageColumns.SUBJECT_SORT_KEY + " text, "
            + MessageColumns.DISPLAY_NAME_SORT_KEY + " text"
            + ");";
    ...
    private static void upgradeBodyFromVersion100ToVersion101(final Context context,
            final SQLiteDatabase db) {
        Cursor cursor = null; //Modify for bug1179190
        try {
            ...
            cursor = db.query(Body.TABLE_NAME, projection,
                    null, null, null, null, null); //Modify for bug1179190
            if (cursor == null) {
            ...
            while (cursor.moveToNext()) {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(htmlString)) {
                        ...
                        /* UNISOC: Modify for bug1208625 @{ */
                        final OutputStreamWriter w = new OutputStreamWriter(new FileOutputStream(htmlFile), Charset.defaultCharset().name());
                        /* @} */
                        try {
                            w.write(htmlString);
                        } finally {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(textString)) {
                        ...
                        /* UNISOC: Modify for bug1208625 @{ */
                        final OutputStreamWriter w = new OutputStreamWriter(new FileOutputStream(textFile), Charset.defaultCharset().name());
                        /* @} */
                        try {
                            w.write(textString);
                        } finally {
    ...
    protected static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion <= 128) {
                upgradeFromVersion127ToVersion128(mContext, db);
            }

            if (oldVersion < 129) {
                upgradeFromVersion128ToVersion129(mContext, db);
            }
        }
    ...
    private static void upgradeFromVersion127ToVersion128(final Context context,
            final SQLiteDatabase db) {
        try {
            createVipMemberTable(db);
        } catch (SQLException e) {
    ...
    private static void upgradeFromVersion128ToVersion129(final Context context,
            final SQLiteDatabase db) {
        addSortKeyColumnsToMessageTable(Message.TABLE_NAME, context, db);
        addSortKeyColumnsToMessageTable(Message.UPDATED_TABLE_NAME, context, db);
        addSortKeyColumnsToMessageTable(Message.DELETED_TABLE_NAME, context, db);
    }

    private static void addSortKeyColumnsToMessageTable(String tableName, final Context context,
            final SQLiteDatabase db) {

        Cursor cursor = null;
        try {
            db.execSQL("alter table " + tableName
                    + " add column " + MessageColumns.SUBJECT_SORT_KEY + " text;");
            db.execSQL("alter table " + tableName
                    + " add column " + MessageColumns.DISPLAY_NAME_SORT_KEY + " text;");
            final String projection[] = {
                    MessageColumns.SUBJECT,
                    MessageColumns.DISPLAY_NAME,
                    BaseColumns._ID
            };
            cursor = db.query(Message.TABLE_NAME, projection,
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                do {
                    ...
                    if (values.size() > 0) {
                        ...
                        db.update(tableName, values, BaseColumns._ID + "=?",
                                new String[] {
                                    Long.toString(id)
                                });
            ...
            LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 128 to 129 ", e);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
