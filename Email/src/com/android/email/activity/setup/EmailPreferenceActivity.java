...
import com.sprd.mail.vip.activity.VipListActivity;
...
public class EmailPreferenceActivity extends MailPreferenceActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        /* SPRD: Bug 492725, when not support feedback, hide the feedback menu @{ */
        if (getResources().getBoolean(R.bool.feedback_supported)) {
    ...
    public void onHeaderClick(@NonNull Header header, int position) {
        ...
        if (header.id == R.id.add_account_header) {
            onAddNewAccount();
            return;
        } else if (header.id == VIP_HEAD_ID) {
            final Intent vipActivityIntent = VipListActivity.createIntent(this,
                    Account.ACCOUNT_ID_COMBINED_VIEW);
            startActivity(vipActivityIntent);
            return;
        }
