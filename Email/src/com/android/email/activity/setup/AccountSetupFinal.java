...
import com.android.email.activity.setup.CheckSettingsErrorDialogFragment.ErrorMessage;
...
public class AccountSetupFinal extends AccountSetupActivity
        ...
        AccountSetupGuideFragment.Callback {
    ...
    /* SPRD: Modify for 848900 {@ */
    private static final String SAVESTATE_KEY_IS_EXCHANGE_SELECTED_IN_GUIDE = "AccountSetupFinal.isExchangeSelectedInGuide";
    /* @} */
    ...
    /* SPRD:bug540863 add begin @{ */
    private static final String SAVESTATE_KEY_ACCOUNT_CREATION_COMPLETE = "AccountCreationFragmentComplete";
    private boolean mAccountCreationComplete = false;
    /* @} */
    ...
    private static final int STATE_GUIDE = 15;

    private static final int REQUEST_EXCHANGE_PERMISSION = 99;

    //private int mState = STATE_BASICS;
    private int mState = UiUtilities.GUIDE_SUPPORT ? STATE_GUIDE : STATE_BASICS;
    /* @} */
    ...
    /* SPRD: Modify for bug1146361 {@ */
    private static boolean mSetupFromCompose = false;
    /* @} */

    private boolean mIsExchangeSelectedInGuide = false;
    @Override
    public void onCreate(Bundle savedInstanceState) {
        ...
        mSetupFromCompose = intent.getBooleanExtra(ComposeActivity.EXTRA_SHARE, false);
        /* @} */
        ...
        if (savedInstanceState != null) {
            ...
            /* SPRD: Modify for 848900 {@ */
            mIsExchangeSelectedInGuide = savedInstanceState
                    .getBoolean(SAVESTATE_KEY_IS_EXCHANGE_SELECTED_IN_GUIDE);
            /* @} */
            ...
            /* SPRD:bug540863 add begin @{ */
            mAccountCreationComplete = savedInstanceState
                    .getBoolean(SAVESTATE_KEY_ACCOUNT_CREATION_COMPLETE);
            if (mAccountCreationComplete) {
                setResult(RESULT_OK);
            }
            /* @} */
        } else {
            ...
            mState = UiUtilities.GUIDE_SUPPORT ? STATE_GUIDE : STATE_BASICS;  //SPRD:bug471289 add account guide function
    ...
    /* UNISOC modify for bug1173144 {@ */
    @Override
    protected void onPostResume() {
        super.onPostResume();
        LogUtils.d(LogUtils.TAG, "AccountSetupFinal onPostResume ");
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        /* SPRD: Modify for 848900 {@ */
        outState.putBoolean(SAVESTATE_KEY_IS_EXCHANGE_SELECTED_IN_GUIDE,
                mIsExchangeSelectedInGuide);
        /* @} */
        outState.putBoolean(SAVESTATE_KEY_PASSWORD_FAILED, mPasswordFailed);
        /* SPRD:bug540863 add begin @{ */
        outState.putBoolean(SAVESTATE_KEY_ACCOUNT_CREATION_COMPLETE, mAccountCreationComplete);
        /* @} */
    }
    ...
    private void updateContentFragment(boolean addToBackstack) {
        ...
        switch (mState) {
            /* SPRD:bug471289 add account guide function {@ */
            case STATE_GUIDE:
                f = AccountSetupGuideFragment.newInstance();
                break;
            /*@}*/
            case STATE_BASICS:
                f = AccountSetupBasicsFragment.newInstance();
                Bundle bundle = new Bundle();
                bundle.putBoolean(AccountSetupBasicsFragment.KEY_SHOW_MANUAL,
                        !mIsExchangeSelectedInGuide);
            ...
            case STATE_NAMES:
                /*
                 * SPRD: 523599 add for account setup auto-completing feature.Modify for bug712291
                 * @{
                 */
                String emailAddress = mSetupData.getEmail();
                ArrayList<String> addressList = new ArrayList<String>();
                addressList.add(emailAddress);
                AccountSettingsUtils.saveAddressToHistoryDB(this, addressList,
    ...
    protected void proceed() {
        ...
        if (oldContentFragment != null) {
            /* SPRD: Add for bug492837 @{ */
            if (mState == STATE_BASICS) {
                oldContentFragment.setNextButtonEnabled(false);
            } else {
        ...
        switch (mState) {
            /* SPRD:bug471289 add account guide function {@ */
            case STATE_GUIDE: {
                final AccountSetupGuideFragment f = (AccountSetupGuideFragment) getContentFragment();
                mIsExchangeSelectedInGuide = f.isExchangeSelected();
                mState = STATE_BASICS;
                ...
                if (shouldDivertToManual()) {
                    ...
                    if (mIsPreConfiguredProvider) {
                        if (!TextUtils.isEmpty(mSetupData.getAmProtocol()) &&
                            ...
                            mState = STATE_AB;
                        } else if (mIsExchangeSelectedInGuide
                                && !"eas".equals(mSetupData.getIncomingProtocol(this))) {
                            mIsPreConfiguredProvider = false;
                        } else {
                    ...
                    if (!mIsPreConfiguredProvider) {
                        String amProtocol = mSetupData.getAmProtocol();
                        if (TextUtils.isEmpty(amProtocol) && mIsExchangeSelectedInGuide) {
                            amProtocol = "eas";
                        }
            ...
            case STATE_AB:
                ...
                break;
            case STATE_CREDENTIALS:
                /* SPRD:bug520850 add autodiscovery function {@ */
                if("eas".equals(mSetupData.getIncomingProtocol(this))){
                    mSkipAutoDiscover = false;
                }
                /*@}*/
                collectCredentials();
            ...
            case STATE_CHECKING_INCOMING:
                ...
                if (serviceInfo.usesSmtp) {
                    mState = STATE_MANUAL_OUTGOING;
                    /* SPRD:bug471289 Modify default server config {@ */
                    if(getContentFragment() instanceof AccountSetupIncomingFragment) {
                        ((AccountSetupIncomingFragment)getContentFragment()).saveSettingsAfterSetup(this, mSetupData);
                    }
                    /* @} */
                } else {
                    mState = STATE_OPTIONS;
    ...
    public void onNextButton() {
        /* SPRD:bug530138 add begin @{ */
        final String existingAccountName =
            mExistingAccountsMap != null ? mExistingAccountsMap.get(mSetupData.getEmail()) : null;
        if (!TextUtils.isEmpty(existingAccountName) && mState == STATE_OPTIONS) {
        ...
        if (!mIsProcessing) {
            if(OsUtils.LEGACY_SCHEME_EAS.equals(mSetupData.getIncomingProtocol(this)) && mState == STATE_CREDENTIALS){
    ...
    private void startRequestPermission() {
        /* SPRD modify for bug718096{@ */
        try {
            final Intent intent = new Intent(OsUtils.EXCHANGE_PERMISSION_REQUEST_INTENT);
            startActivityForResult(intent, REQUEST_EXCHANGE_PERMISSION);
        } catch (ActivityNotFoundException e) {
            Toast.makeText(this, R.string.exchange_need_to_be_enabled, Toast.LENGTH_SHORT)
                    .show();
            LogUtils.e(LogUtils.TAG, "startRequestPermission exchange diasabled");
        }
        /* @} */
    }
    ...
    public void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        if (requestCode == REQUEST_EXCHANGE_PERMISSION) {
    ...
    private boolean onBasicsComplete() {
         ...
         */
        final Account account = mSetupData.getAccount();
        setUserSignatureFlag(account);
        /*@}*/
        final String[] emailParts = email.split("@");
        /* SPRD: Modify bug496485, Monkey test ArrayIndexOutOfBoundsException. { */
    ...
    private void showDuplicateAccountDialog(final String existingAccountName) {
        ///SPRD Bug497990 should disable "Next" button while showing this dialog.
        getContentFragment().blockNextButton();
    ...
    private boolean finishAutoSetup() {
        ...
        try {
            ...
            /* SPRD: Add for bug552382 @{ */
            if (info != null && !info.isGmailStub && !EmailServiceUtils.isServiceAvailable(this, info.protocol)) {
            ...
            if (info == null) {
                LogUtils.d(LogUtils.TAG, "EmailServiceInfo equals null");
                return false;
            }
            /* }@ */
        ...
        /* SPRD: Modify for bug567743 {@ */
        } catch (ArrayIndexOutOfBoundsException e) {
            LogUtils.e(LogUtils.TAG, e, "Email address may contain wrong characters");
            Toast.makeText(this, R.string.email_address_wrong, Toast.LENGTH_SHORT).show();
            return false;
        /* @} */
        }
        return true;
    ...
    private void onIncomingComplete() {
        /* SPRD: Modify for bug496706, ClassCastException.
         * original code:
        ...
        if (f instanceof AccountSetupIncomingFragment) {
            ...
            asif.collectUserInput();
        } else {
            LogUtils.wtf(LogUtils.TAG, "Tried to collect user input on non-incoming screen");
        }
        /*@}*/
    }
    ...
    private void onOutgoingComplete() {
         ...
         */
        AccountSetupFragment f = getContentFragment();
        if (f instanceof AccountSetupOutgoingFragment) {
            AccountSetupOutgoingFragment asof = (AccountSetupOutgoingFragment) f;
            asof.collectUserInput();
        } else {
            LogUtils.wtf(LogUtils.TAG, "Tried to collect user input on non-outgoing screen");
        }
        /*@}*/
    }
    ...
    private void populateHostAuthsFromSetupData() {
        ...
        final EmailServiceUtils.EmailServiceInfo info =
                mSetupData.getIncomingServiceInfo(this);
        /* UNISOC: Modify for bug1089093 bug1095075{@ */
        if (info == null) {
            LogUtils.e(LogUtils.TAG, "getIncomingServiceInfo " + email + " is null");
            mState = STATE_BASICS;
            return;
        }
        /* @} */
        ...
        recvAuth.setConnection(mSetupData.getIncomingProtocol(), domain,
                HostAuth.PORT_UNKNOWN, info.offerTls ? HostAuth.FLAG_NONE : HostAuth.FLAG_SSL);
        /*@}*/
        AccountSetupCredentialsFragment.populateHostAuthWithResults(this, recvAuth,
                mSetupData.getCredentialResults());
        mSetupData.setIncomingCredLoaded(true);
    ...
    private void initiateCheckSettingsFragment(int checkMode) {
        ...
        getFragmentManager().beginTransaction()
                ...
                /* SPRD: Bug509980, IllegalStateException after onSaveInstanceState.{@ */
    ...
    private void dismissCheckSettingsFragment() {
        ...
        /* SPRD: Modify for bug691274 @{ */
        if (f != null && d != null) {
            getFragmentManager().beginTransaction()
                .remove(f)
                .remove(d)
                /* SPRD:bug603441 modify begin @{ */
                .commitAllowingStateLoss();
                /* @} */
        }
        /* @} */
    ...
    /* SPRD modify for bug694376 {@ */
    @Override
    public void onCheckSettingsError(int reason, ErrorMessage message) {
        ...
        // / SPRD bug497990 Block quick press on "Next" button here, to avoid wrong proceed between
        // fragment
        // dismiss and show, cause fragment shows not immediately @{
        getContentFragment().blockNextButton();
        // / @}
        dismissCheckSettingsFragment();
        final DialogFragment f =
                CheckSettingsErrorDialogFragment.newInstance(reason, message);
        /* SPRD: Modify for bug 819937  @{ */
        f.showAllowingStateLoss(getFragmentManager(), CheckSettingsErrorDialogFragment.TAG);
        /* @} */
    }
    /* @} */
    ...
    public void onCheckSettingsErrorDialogEditSettings() {
        ...
        /* SPRD: Modify for bug842596 @{ */
        AccountSetupFragment f = getContentFragment();

        if (mState == STATE_CHECKING_PRECONFIGURED) {
            mPreConfiguredFailed = true;
            proceed();
        } else if (f != null && f instanceof AccountSetupCredentialsFragment) {
            mState = STATE_MANUAL_INCOMING;
            updateContentFragment(true);
        } else {
            resetStateFromCurrentFragment();
    ...
    @Override
    public void onCheckSettingsAutoDiscoverComplete(HostAuth autoDiscoverResult) {
        /* SPRD:bug520850 add autodiscovery function {@ */
        final Account account = mSetupData.getAccount();
        final HostAuth recvAuth = account.getOrCreateHostAuthRecv(this);
        LogUtils.d(LogUtils.TAG,"autoDiscoverResult " + autoDiscoverResult);
        if (autoDiscoverResult != null) {
            mState = STATE_MANUAL_INCOMING;
            recvAuth.mAddress = autoDiscoverResult.mAddress;
            recvAuth.mPort = autoDiscoverResult.mPort;
            recvAuth.mFlags = autoDiscoverResult.mFlags;
        }
        /*@}*/
        dismissCheckSettingsFragment();
        proceed();
    ...
    private void initiateAccountCreation() {
        ...
        /* UNISOC: Modify for Bug1082085 {@ */
        account.setSenderName(account.getEmailAddress());
        /* @} */
        ...
        if (hostAuth.mProtocol.equals(getString(R.string.protocol_eas))) {
            try {
                /* SPRD:bug492790 modify begin @{ */
                final String protocolVersion = TextUtils.isEmpty(account.mProtocolVersion) ? ""
                        : account.mProtocolVersion;
                final double protocolVersionDouble = Double.parseDouble(protocolVersion);
                /* @} */
        ...
        /* SPRD:bug499554 add begin @{ */
        mSetupData.setEmailAddress(account.mHostAuthRecv.mLogin);
        /* @} */
    ...
    public void onAccountFinalizeFragmentComplete() {
        /* UNISOC: Modify for bug 1146361 {@ */
        if (mSetupFromCompose){
            Toast.makeText(this, R.string.account_setup_from_compose, Toast.LENGTH_SHORT)
                .show();
        }
        /* @} */
        finish();
     ...
     *@{
     */
    private void setUserSignatureFlag(Account account){
        String signature = getResources().getString(R.string.signature_flag);
