...
public class AccountSettingsFragment extends MailAccountPrefsFragment
    ...
    /* @} */
    /* SPRD modify for bug697075{@ */
    private boolean mIsRingTonePickerLaunching = false;
    /* @} */
    /**
    ...
    /* SPRD: Modify for bug839458 @{ */
    @Override
    public void onDetach() {
        FragmentManager fm = getFragmentManager();
        Fragment OofWaitingFragment = fm.findFragmentByTag(OofGetWaitingFragment.TAG);

        if (OofWaitingFragment != null) {
            FragmentTransaction fragmentTransaction = fm.beginTransaction();
            fragmentTransaction.remove(OofWaitingFragment);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /* SPRD: Modify for bug680748, bug688940 @{ */
        if (savedInstanceState != null) {
            ...
            final String syncIntervalStrings =
                    savedInstanceState.getString(SAVESTATE_SYNC_INTERVAL_STRINGS);
            final String syncIntervals =
                    savedInstanceState.getString(SAVESTATE_SYNC_INTERVALS);
            mCheckFrequency = (ListPreference) findPreference(PREFERENCE_FREQUENCY);
            if (mCheckFrequency != null && !TextUtils.isEmpty(syncIntervalStrings) && !TextUtils.isEmpty(syncIntervals) ) {
                mCheckFrequency.setEntries(Utils.getArrayResourceIdByFilter(mContext, syncIntervalStrings));
                mCheckFrequency.setEntryValues(Utils.getArrayResourceIdByFilter(mContext, syncIntervals));
            }
        }
        /* @} */

        /* SPRD modif for bug713496 {@ */
    ...
    public void onSaveInstanceState(@NonNull Bundle outstate) {
        ...
        if (mServiceInfo != null) {
            outstate.putString(SAVESTATE_SYNC_INTERVAL_STRINGS,
                    mServiceInfo.syncIntervalStrings);
            outstate.putString(SAVESTATE_SYNC_INTERVALS,
                    mServiceInfo.syncIntervals);
        }
        /* @} */
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            case RINGTONE_REQUEST_CODE:
                /* SPRD modify for bug697075{@ */
                LogUtils.i(LogUtils.TAG, "onActivityResult RINGTONE_REQUEST_CODE ");
                mIsRingTonePickerLaunching = false;
                /* @} */
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        /* SPRD: Modify for bug873799 @{ */
        /* SPRD: Modify for bug704173 @{ */
        if (mAccount == null || mUiAccount == null || mInboxFolder == null) {
        /* @} */
            if (mToast != null) {
                mToast.cancel();
            }
            mToast = Toast.makeText(mContext,
                    getResources().getString(R.string.wait_for_sync_title), Toast.LENGTH_SHORT);
            if (mToast != null) {
                mToast.show();
            }
            return true;
        ...
        switch (key) {
            ...
            case PREFERENCE_SIGNATURE:
                result = super.onPreferenceTreeClick(preferenceScreen, preference);
                edit = ((EditTextPreference) findPreference(key)).getEditText();
                text = edit.getText().toString();
                if (!TextUtils.isEmpty(text)) {
                    edit.setSelection(text.length());
                }
                break;
            case PREFERENCE_SYNC_SETTINGS:
                startActivity(MailboxSettings.getIntent(getActivity(), mUiAccount.fullFolderListUri,
                        mInboxFolder));
                break;
            case PREFERENCE_OOF_SETTINGS:
                onOutOfOffice(mAccount);
                break;
            case PREFERENCE_QUICK_RESPONSES:
                onEditQuickResponses(mUiAccount);
                break;
            case PREFERENCE_POLICIES_RETRY_ACCOUNT:
                SecurityPolicy.setAccountHoldFlag(mContext, mAccount, false);
                break;
            case PREFERENCE_INCOMING:
                onIncomingSettings(mAccount);
                break;
            case PREFERENCE_OUTGOING:
                onOutgoingSettings(mAccount);
                break;
            case PREFERENCE_DELETE_ACCOUNT:
                // SPRD:Delete account function from AccountSettings @{
                // SPRD: for bug 496007 IllegalStateException: Can not perform
                // this action after onSaveInstanceState.
                /* SPRD: Modify for bug 725672, 839458 @{ */
                FragmentManager fm = getFragmentManager();
                if (fm != null) {
                    fm.executePendingTransactions();

                    Activity activity = getActivity();
                    Fragment fragment = fm.findFragmentByTag(DeleteAccountFragment.TAG);

                    if (activity != null && activity.isResumed() && fragment == null
                            && !this.isDetached()) {
                        DeleteAccountFragment dialogFragment = DeleteAccountFragment
                                .newInstance(mAccount, AccountSettingsFragment.this);
    ...
    private void loadSettings() {
        ...
        /* SPRD Modify for bug877274 {@ */
        if (ActivityManager.isUserAMonkey() && !isAdded()) {
            LogUtils.e(LogUtils.TAG, "Ignore set signature after AccountSettingsFragment is detached from EmailPreferenceActivity");
            return;
        }
        /* @} */
        ...
        /* SPRD: Modify for bug680748 @{ */
        mCheckFrequency.setEntries(Utils.getArrayResourceIdByFilter(mContext, mServiceInfo.syncIntervalStrings));
        mCheckFrequency.setEntryValues(Utils.getArrayResourceIdByFilter(mContext, mServiceInfo.syncIntervals));
        /* @} */
        ...
        if (mServiceInfo.offerLookback) {
            ...
            /* SPRD: Modify for bug704173 @{ */
            if (mSyncSettings == null && mInboxFolder != null) {
        ...
        if (mInboxFolderPreferences != null) {
            ...
            if (mContext.getApplicationInfo().targetSdkVersion >= android.os.Build.VERSION_CODES.O) {
                /* UNISOC: Modify for bug1122284 {@ */
                if (null != mInboxRingtone) notificationsCategory.removePreference(mInboxRingtone);
                if (null != mInboxVibrate) notificationsCategory.removePreference(mInboxVibrate);
                /* @}*/
            }
        } else {
    ...
    */
    public static class DeleteAccountFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            /* SPRD: Modify for bug839458 @{ */
            return new AlertDialog.Builder(context).setTitle(R.string.account_delete_dlg_title)
                    .setMessage(
                            context.getString(R.string.account_delete_dlg_instructions_fmt, name))
    ...
    */
    private void finishDeleteAccount() {
        if (OsUtils.LEGACY_SCHEME_EAS.equals(mAccount.getProtocol(mContext))) {
            /* SPRD modify for bug711630 {@ */
            PackageManager pm = mContext.getPackageManager();
            final Intent intent = new Intent(OsUtils.EXCHANGE_PERMISSION_REQUEST_INTENT);
            if (pm.resolveActivity(intent, 0) == null) {
                LogUtils.i(LogUtils.TAG,
                        "finishDeleteAccount Exchange Application Might be disabled");
                Toast.makeText(mContext, mContext.getString(R.string.exchange_need_to_be_enabled),
    ...
    private void showRingtonePicker() {
        /* SPRD modify for bug697075{@ */
        LogUtils.i(LogUtils.TAG, "showRingtonePicker isRingTonePickerLaunching: "
                + mIsRingTonePickerLaunching);
        if (!mIsRingTonePickerLaunching) {
            mIsRingTonePickerLaunching = true;
    ...
    public void deleteAccount(Context mContext, Account account) {
        ...
        new Thread(new Runnable() {
            public void run() {
                mContext.getContentResolver().delete(mUiAccount.uri, null, null);
                mContext.getContentResolver().delete(Account.CONTENT_URI,
                        EmailContent.ID_SELECTION, new String[]{Long.toString(account.mId)});
    ...
    /*
     * SPRD: bug523600 add OOF function,Start the oof waiting fragment for get the out of office
     * settings.{@
     */
    private void startOofGetWaitingFragment(long accountId, AccountSettingsFragment target) {
        ...
        if (fm != null) {
            ...
            // if the targetfragment is detach, we should return too
            /* SPRD: Modify for bug690614 @{ */
            if (fm.findFragmentByTag(OofGetWaitingFragment.TAG) != null
                    || target.isDetached()
                    || getActivity() == null) {
                return;
            }
            /* @} */
            OofGetWaitingFragment checkerFragment = OofGetWaitingFragment.newInstance(accountId,
                    target);
            FragmentTransaction transaction = fm.beginTransaction();
            transaction.add(checkerFragment, OofGetWaitingFragment.TAG);
            /* SPRD: Modify for bug676340 @{ */
