...
public class EmailOmacpService extends IntentService {
    /* SPRD modify for bug677524{@ */
    public EmailOmacpService() {
        super("EmailOmacpService");
    }
    /* @} */
    ...
    /* UNISOC: Modify for bug 1146879 {@ */
    public static int NOTIFICATION_ID = 0x10000000;
    public static String NOTIFICATION_CHANNEL = "OMACP_SERVICE_NOTIFICATION_CHANNEL";
    /* @} */
     ...
     */
    /* SPRD modify for bug677524{@ */
    private boolean checkEmailServerConnect(final Context context, final Account account) {
    ...
    private void addAccount(Account account) {
        ...
        //update account flag.
        /* SPRD: Modify for bug719752 @{ */
        String protocol = account.mHostAuthRecv == null ? null : account.mHostAuthRecv.mProtocol;
        if (null != protocol) {
            setFlagsForProtocol(account, protocol);
        } else {
            Log.d(TAG, "protocol is null, we must set account with one protocol !!!");
            return;
        }
        /* @} */
        ...
        if (!findExistingAccount(this, mFrom)) {
            ...
            if ("eas".equals(protocol)) {
                ...
                EmailServiceUtils.startService(this, protocol);
            }
            /* @} */
    ...
    protected void onHandleIntent(Intent intent) {
        /* UNISOC: Modify for bug 1146879 {@ */
        final NotificationManager nmchannel = this.getSystemService(NotificationManager.class);
        ...
        nmchannel.createNotificationChannel(channel);
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this);
        builder.setContentTitle(this.getString(R.string.app_name)); //UNISOC: Modify for bug 1166156
        builder.setChannelId(NOTIFICATION_CHANNEL);
        builder.setSmallIcon(R.drawable.email_notification_icon); //UNISOC: Modify for bug 1157277
        startForeground(NOTIFICATION_ID, builder.build());
        /* @} */

        OmacpSyncUtils.acquireLock();
        Log.d(TAG, "onHandleIntent intent:" + intent);
        Intent omacpIntent = intent.getParcelableExtra(Intent.EXTRA_INTENT);
        Account account = (omacpIntent == null) ? null : parserOmacpRequest(omacpIntent);
        Log.d(TAG, "account is " + account);
        if (account != null) {
            boolean shouldAddAccount = true;
            if (NEED_CHECK_NET) {
                shouldAddAccount = checkEmailServerConnect(this, account);
            }
            ...
            sendResultToOmacp(shouldAddAccount);
        } else {
            sendResultToOmacp(false);
        }
        OmacpSyncUtils.realseLock();
    }
    /* @} */
}
