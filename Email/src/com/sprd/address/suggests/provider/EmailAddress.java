...
public final class EmailAddress extends EmailAddressContent implements
    ...
    private static void saveAddress(ContentResolver resolver, String text, String type) {
        ContentValues cv = new ContentValues();
        cv.put(AddressColumns.EMAIL_ADDRESS, text);
        cv.put(AddressColumns.ADDRESS_TYPE, type);
        Uri res = resolver.insert(CONTENT_URI, cv);
        if (res == null) {
    ...
    /* SPRD modify for bug712291,812362 {@ */
    public static void saveAddressList(Context context, List<String> addressList, String type) {
        ContentResolver resolver = context.getContentResolver();
        int addresCount = addressList.size();
        for (int i = 0; i < addresCount; i++) {
            saveAddress(resolver, addressList.get(i), type);
        }
    }
