...
public class Rfc822Output {
    ...
    private static final String DATE_FORMAT = "EEE, dd MMM yyyy HH:mm:ss Z";//UNISOC:Modify for bug1208377
    ...
    public static void writeTo(Context context, Message message, OutputStream out,
        ...
        final Writer writer = new OutputStreamWriter(stream, Charset.defaultCharset().name());//UNISOC:Modify for bug1208377

        // Write the fixed headers.  Ordering is arbitrary (the legacy code iterated through a
        // hashmap here).

        final String date = new SimpleDateFormat(DATE_FORMAT, Locale.US).format(new Date(message.mTimeStamp));//UNISOC:Modify for bug1208377
        writeHeader(writer, "Date", date);

        writeEncodedHeader(writer, "Subject", message.mSubject);

        /* UNISOC: Modify for bug1429702 {@ */
        if (TextUtils.isEmpty(message.mMessageId)) {
            writeHeader(writer, "Message-ID", MimeMessage.generateMessageId());
        } else {
        ...
        if (UIProvider.ENABLE_MAIL_PRIORITY_FEATURE) {
            if ((message.mFlags & Message.FLAG_TYPE_PRIORITY_HIGH) != 0) {
                writeHeader(writer, "X-Priority", "1");
                writeHeader(writer, "Importance", "high");
                LogUtils.i(TAG, "[Rfc822Output][writeTo]X-Priority:1 Importance:high");
            } else if ((message.mFlags & Message.FLAG_TYPE_PRIORITY_LOW) != 0) {
                writeHeader(writer, "X-Priority", "5");
                writeHeader(writer, "Importance", "low");
                LogUtils.i(TAG, "[Rfc822Output][writeTo]X-Priority:5 Importance:low");
            }
        }
        /*@}*/
    ...
    private static void writeOneAttachment(Context context, Writer writer, OutputStream out,
        ...
        /* SPRD: Modify for bug499385 {@ */
        writeEncodedHeaderNew(writer, "Content-Type: " + attachment.mMimeType + "; ", "\n name=\"",
        ...
        if ((attachment.mFlags & Attachment.FLAG_ICS_ALTERNATIVE_PART) == 0) {
            /* SPRD: Modify for bug499385 {@ */
            writeEncodedHeaderNew(writer, "Content-Disposition: attachment; ", "filename=\"",
                    attachment.mFileName, "\"; size=" + Long.toString(attachment.mSize));
            /* @} */
        }
    ...
    private static void writeTextWithHeaders(Writer writer, OutputStream out, String[] bodyText)
            throws IOException {
        /* SPRD: Modify for bug 541340 if we have text and html content,
         * we use multipart/alternative as content-type {@ */
        String text = bodyText[INDEX_BODY_TEXT];
        String html = bodyText[INDEX_BODY_HTML];
        boolean multipart = false;
        boolean replaceText = false;
        if (!TextUtils.isEmpty(text) && !TextUtils.isEmpty(html)) {
            multipart = true;
        ...
        if (TextUtils.isEmpty(text)) {
            ...
            final String mimeType = "text/"
                    + ((multipart || !replaceText) ? ("plain") : ("html"));
    ...
    /* SPRD: Modify for bug499385 {@ */
    private static void writeEncodedHeaderNew(Writer writer, String str1, String name,
            String value, String str2) throws IOException {
        if (value != null && value.length() > 0) {
            writer.append(str1 + name);
            writer.append(MimeUtility.foldAndEncode2(value, name.length() + 2));
            writer.append(str2);
            writer.append("\r\n");
        }
