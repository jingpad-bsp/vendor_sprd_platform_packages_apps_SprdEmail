...
public class Device {
    ...
    static private String getDeviceIdInternal(Context context) throws IOException {
        ...
        if (f.exists()) {
            if (f.canRead()) {
                /* UNISOC: Modify for bug 1208363 {@ */
                try (BufferedReader rdr = new BufferedReader(new FileReader(f), 128)) {
                    id = rdr.readLine();
                }
                /* @}  */
                if (id == null) {
        ...
        /* UNISOC: Modify for bug 1208363 {@ */
        try (BufferedWriter w = new BufferedWriter(new FileWriter(f), 128)) {
            w.write(id);
        }
        /* @}  */
        return id;
