...
import com.sprd.mail.vip.utils.ClosingMatrixCursor;

import java.util.HashSet;

/**
 * SPRD: Cache the vip members in memory to improve the performance
 *
 */
public class VipMemberCache {
    ...
    private static VipMemberCache sInstance = null;//UNISOC:Modify for bug1208573
    ...
    private VipMemberCache(Context context) {
        LogUtils.d(TAG, "VipMemberCache init...");
        mContext = context;
        mUpdateRunnable = new UpdateRunnable();
        mContentObserver = new VipContentObserver(new Handler(), mContext, mUpdateRunnable);
        ///SPRD: only VipMember's change should callback from this contentObserver.Performance optimize
    ...
    public static boolean isVIP(String fromList) {
        ...
        ///SPRD: keep sync between multi-thread access
        synchronized (sVipAddresses) {
    ...
    public static Cursor getVipMessagesIds(Context context, long accountId, boolean onlyUnread) {
        ...
        if (c == null) {
            LogUtils.e(TAG, "getVipMessagesIds return empty cursor because cursor is null");
            return new MatrixCursor(EmailContent.ID_PROJECTION);
        }
    ...
    /** SPRD: get the unread Vip msg count for update @{ */
    ...
    private class UpdateRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                /// SPRD: Turn "sync to network" parameter to "false" as this operation is unnecessary to upsync
                ...
                LogUtils.w(TAG, "Can not update VipMemberCache", ex);
            } finally {
                if (c != null) {
                    c.close();
                }
    ...
    private static class VipContentObserver extends ContentObserver implements Runnable {
        ...
        public void register(Uri notifyUri) {
            ...
            mRegistered = true;
            LogUtils.d(TAG, "VipContentObserver register");
        }
    ...
    /**
     * SPRD: Support for VIP folder, filter messages of VIP folder
