...
public class FolderPreferences extends VersionedPrefs {
    ...
    public static final class PreferenceKeys {
        ...
        public static final String SORT_ORDERY_KEY = "sort_order_key";
    ...
    public String getNotificationRingtoneUri() {
        ...
        try {
            cursor = res.query(Uri.parse(ringtoneUri), new String[]{MediaStore.Audio.Media._ID}, null, null, null);
        } catch (SecurityException e) {
            LogUtils.i(LogUtils.TAG, "SecurityException when ContentResolver query" + e.toString());
        } catch (SQLiteException e) {
            LogUtils.i(LogUtils.TAG, "SQLiteException when ContentResolver query" + e.toString());
        }
        try {
    ...
    /* SPRD: Modify for bug508106 534337{@ */
    public String getActualNotificationRingtoneUri() {
        String uri = Settings.System.getString(getContext().getContentResolver(),
                Settings.System.NOTIFICATION_SOUND);
        return uri;
    }
    /* SPRD: Modify for bug 598997{@ */
    public static boolean hasDeleted(Context context, Uri uri) {
        if (uri == null) {
            return true;
        }
        ...
        try {
            ...
            LogUtils.w(LOG_TAG, e, "Error opening file");
            return true;
        } finally {
            try {
                if (file != null) {
                ...
                LogUtils.w(LOG_TAG, "Error closing file");
            }
        }
        return false;
    }
    ...
    public int getSortOrder() {
        return getSharedPreferences().getInt(PreferenceKeys.SORT_ORDERY_KEY, 0);
    }

    public void setSortOrder(int sortOrder) {
        getEditor().putInt(PreferenceKeys.SORT_ORDERY_KEY, sortOrder).apply();
        notifyBackupPreferenceChanged();
    }

    public boolean isEveryMessageNotificationEnabled() {
        /* Modify for bug 641911 {@ */
        return getSharedPreferences()
                .getBoolean(PreferenceKeys.NOTIFICATION_NOTIFY_EVERY_MESSAGE, true);
        /* @} */
    }
