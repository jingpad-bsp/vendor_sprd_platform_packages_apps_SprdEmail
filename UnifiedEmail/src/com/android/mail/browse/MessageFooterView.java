...
public class MessageFooterView extends LinearLayout implements DetachListener,
    ...
    private void renderBarAttachments(List<Attachment> barAttachments, boolean loaderResult) {
        ...
        for (Attachment attachment : barAttachments) {
            ...
            if ((attachment.flags & Attachment.FLAG_DUMMY_ATTACHMENT) != 0 && (attachment.flags
                    & Attachment.FLAG_INVALID_TO_DOWNLOAD) != 0) {
                continue;
            }
            /* @} */
    ...
    public void onDetachedFromParent() {
        int mCount = mAttachmentBarList.getChildCount();
        for (int count = 0; count < mCount; count++) {
            MessageAttachmentBar barAttachmentView = (MessageAttachmentBar) mAttachmentBarList.getChildAt(count);
            barAttachmentView.popupMenuDismiss();
        }
    }
    /* @} */
