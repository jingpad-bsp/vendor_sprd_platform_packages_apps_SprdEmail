...
import com.sprd.mail.DownloadConfirmDialogFragment;
...
public class AttachmentActionHandler {
    ...
    public void showDownloadingDialog() {
        ...
        /* SPRD: bug492737 modify begin @{ */
        if (prev == null) {
            // Create and show the dialog.
            final DialogFragment newFragment = AttachmentProgressDialogFragment
                    .newInstance(mAttachment);
            newFragment.show(ft, PROGRESS_FRAGMENT_TAG);
        } else {
            final AttachmentProgressDialogFragment dialog = (AttachmentProgressDialogFragment) prev;
            if (dialog != null && !dialog.isShowingDialogForAttachment(mAttachment)) {
                dialog.show(ft, PROGRESS_FRAGMENT_TAG);
            }
        }
        /* @} */
    }
     ...
     *@{
     */
    public void showDownloadConfirmDialog(int dialogType, DownloadConfirmInterface cb) {
        showDownloadConfirmDialog(dialogType, cb, AttachmentDestination.CACHE);
    }

    public void showDownloadConfirmDialog(int dialogType, DownloadConfirmInterface cb, int destination) {
        /* SPRD: bug611475 modify begin @{ */
        if (mContext != null && ((Activity) mContext).isResumed()) {
            final DownloadConfirmDialogFragment frag = DownloadConfirmDialogFragment.newInstance(
                    mAttachment, dialogType, destination);
            frag.setCallback(cb);
            frag.show(mFragmentManager, "download confirm");
        }
        /* @} */
    }
    /*@}*/
    ...
    public void updateStatus(boolean loaderResult) {
        ...
        if (dialog != null && dialog.isShowingDialogForAttachment(mAttachment)) {
            /* SPRD:add for bug523469, dialog progress dismiss@{ */
            if (showProgress) {
    ...
    /* SPRD:add for bug499631, dismiss a progress Dialog @{ */
    public void dismissDialog() {
        ...
        if (dialog != null) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    /* SPRD: modify for bug 514474 521781 @{ */
                    try {
                        dialog.dismissAllowingStateLoss();
                    } catch (IllegalStateException e) {
                        LogUtils.e(LOG_TAG, e, "Error while dissmssing diaolg");
                    }
                    /* @} */
                }
            });
