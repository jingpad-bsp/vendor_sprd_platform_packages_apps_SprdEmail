...
public class SendersView {
    ...
    public static void format(Context context, ConversationInfo conversationInfo,
        ...
        try {
            ...
            format(context, conversationInfo, messageInfo, maxChars, styledSenders,
                    displayableSenderNames, displayableSenderEmails, senderAvatarModel, account,
    ...
    private static void handlePriority(int maxChars, String messageInfoString,
        ...
        for (int i = 0; i < conversationInfo.participantInfos.size(); i++) {
            ...
            if (shouldSelectSenders) {
                if (i == 0) {
                    ...
                    if (!Objects.equal(firstSenderEmail, senderEmail)) {
                        ...
                        displayableSenderEmails.add(currentName);
            ...
            if (shouldAddPhotos) {
                String senderEmail2 = TextUtils.isEmpty(currentName) ? account.getEmailAddress()
                        : TextUtils.isEmpty(currentEmail) ? currentName : currentEmail;
                if (i == 0) {
                    // Always add the first sender!
                    firstSenderEmail = senderEmail2;
                    firstSenderName = currentName;
                } else {
                    if (!Objects.equal(firstSenderEmail, senderEmail2)) {
                        int indexOf = displayableSenderEmails.indexOf(senderEmail2);
                        if (indexOf > -1) {
                            displayableSenderEmails.remove(indexOf);
                            displayableSenderNames.remove(indexOf);
                        }
                        displayableSenderEmails.add(senderEmail2);
                        displayableSenderNames.add(currentName);
                        if (displayableSenderEmails.size() > MAX_SENDER_COUNT) {
                            displayableSenderEmails.remove(0);
                            displayableSenderNames.remove(0);
                        }
                    }
        ...
        if (shouldAddPhotos && !TextUtils.isEmpty(firstSenderEmail)) {
            if (displayableSenderEmails.size() < MAX_SENDER_COUNT) {
                displayableSenderEmails.add(0, firstSenderEmail);
                displayableSenderNames.add(0, firstSenderName);
            } else {
                displayableSenderEmails.set(0, firstSenderEmail);
                displayableSenderNames.set(0, firstSenderName);
            }
        }
    ...
    /* SPRD:Add for bug518717 Start {@ */
    public static SpannableString getFormattedToHeader() {
        return getFormattedToHeader(null);
    }

    public static SpannableString getFormattedToHeader(Context context) {
        if (TextUtils.isEmpty(sToHeaderString)) {
            if (context != null) {
                sToHeaderString = context.getResources().getString(R.string.to_heading);
            } else {
                sToHeaderString = "To: ";
            }
        }
        /* SPRD:Add for bug518717 End @} */
