...
public class SubjectAndFolderView extends TextView implements FolderSpan.FolderSpanDimensions {
     ...
     *@{
     */
    private final Drawable mPriorityHighDrawable;
    private final Drawable mPriorityLowDrawable;
    /*@}*/
    ...
    public SubjectAndFolderView(Context context, AttributeSet attrs) {
         ...
         *@{
         */
        mPriorityHighDrawable = res.getDrawable(
                R.drawable.ic_email_caret_double_important_unread);
        mPriorityHighDrawable.setBounds(0, 0, mPriorityHighDrawable.getIntrinsicWidth(),
                mPriorityHighDrawable.getIntrinsicHeight());
        mPriorityLowDrawable = res.getDrawable(
                R.drawable.ic_email_caret_double);
        mPriorityLowDrawable.setBounds(0, 0, mPriorityLowDrawable.getIntrinsicWidth(),
                mPriorityLowDrawable.getIntrinsicHeight());
        /*@}*/
    ...
    public void setFolders(ConversationViewHeaderCallbacks callbacks, Account account,
        ...
        if (UIProvider.ENABLE_MAIL_PRIORITY_FEATURE) {
            final int start2 = sb.length();
            if (UIProvider.MailPriority.HIGH == conv.mailPriority) {
                sb.append("_\u0020");
                sb.setSpan(new ReplacementSpan() {
                    @Override
                    public int getSize(Paint paint, CharSequence text, int start, int end,
                            Paint.FontMetricsInt fm) {
                        return mPriorityHighDrawable.getIntrinsicWidth();
                    }
                    ...
                    public void draw(Canvas canvas, CharSequence text, int start, int end, float x,
                        ...
                        final int transY = baseline + mChipVerticalOffset -
                                mPriorityHighDrawable.getIntrinsicHeight();
                        canvas.translate(x, transY);
                        mPriorityHighDrawable.draw(canvas);
                        canvas.restore();
                    }
                }, start2, start2 + 1, Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
            } else if (UIProvider.MailPriority.LOW == conv.mailPriority) {
                sb.append("_\u0020");
                sb.setSpan(new ReplacementSpan() {
                    @Override
                    public int getSize(Paint paint, CharSequence text, int start, int end,
                            Paint.FontMetricsInt fm) {
                        return mPriorityLowDrawable.getIntrinsicWidth();
                    }
                    ...
                    public void draw(Canvas canvas, CharSequence text, int start, int end, float x,
                        ...
                        final int transY = baseline + mChipVerticalOffset -
                                mPriorityLowDrawable.getIntrinsicHeight();
                        canvas.translate(x, transY);
                        mPriorityLowDrawable.draw(canvas);
                        canvas.restore();
                    }
                }, start2, start2 + 1, Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
            }
