...
public class ConversationPagerAdapter extends FragmentStatePagerAdapter2
        implements ViewPager.OnPageChangeListener, SensorEventListener{
    ...
    public ConversationPagerAdapter(Context context, FragmentManager fm, Account account,
        ...
        mSensor = mSensorManager.getDefaultSensor(Utils.TYPE_SPRDHUB_SHAKE);
    ...
    private ConversationCursor getCursor() {
        ...
        if (mController == null) {
            ...
            notifyDataSetChanged();//UNISOC: Modify for bug 1189530
    ...
    public void notifyDataSetChanged() {
        ...
        /* SPRD: modify for bug630222, bug611475, bug676410 {@ */
        if (mController != null && mPager != null) {
            ...
            final int posFromCursor = getConversationFromCursor(currConversation);
            final ConversationCursor cursor = getCursor();
            LogUtils.i(LOG_TAG, "CPA mDetachedMode:" + mDetachedMode + " pos:" + pos
                    + " posFromCursor:" + posFromCursor);
            ...
            } else if (!mDetachedMode || posFromCursor != POSITION_NONE) {
    ...
    /* SPRD modify for bug676410{@ */
    private int getConversationFromCursor(Conversation conv) {
        if (conv == null) {
            return POSITION_NONE;
        }
        ...
        if (mSingletonMode) {
            ...
            if (!conv.equals(def)) {
                LogUtils.d(
                        LOG_TAG,
                        "in CPA.getConversationFromCursor unable to find conversation in singleton mode. c=%s def=%s",
                        conv, def);
                return POSITION_NONE;
            }
            LogUtils.d(LOG_TAG, "in CPA.getConversationFromCursor returning 0, conv=%s this=%s",
                    conv, this);
            return 0;
        }

        final ConversationCursor cursor = (mController == null) ? null : mController
        ...
        if (pos >= 0) {
            LogUtils.d(
                    LOG_TAG,
                    "in CPA.getConversationFromCursor pager adapter found repositioned convo %s at pos=%d",
                    conv, pos);
            result = pos;
        }

        LogUtils.d(LOG_TAG, "in CPA.getConversationFromCursor (normal), conv=%s pos=%s this=%s",
                conv, result, this);
        return result;
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        synchronized (mLock) {
            ...
            android.util.Log.d("sensor","event:" + z);
            boolean isEnable = getIsShakeSwipeEnable();
            android.util.Log.d("sensor","shakeswipe enable: " + isEnable);
            if(mPager == null || getCount() < 2 || !isEnable){
                android.util.Log.d("sensor","onChange return!!!!");
                return;
            }
            Fragment f = getFragmentAt(mPager.getCurrentItem());
            if(f == null || !f.getActivity().isResumed()){
                android.util.Log.d("sensor","onChange activity not resumed!!!!");
                return;
            }

            /* SPRD: modify for bug637837 {@ */
            if (z == 2) {
                mHandler.post(mShowNextPageRunnable);
            } else if (z == 1) {
                mHandler.post(mShowPrePageRunnable);
            }
            /* @} */
        }
    }

    private final Runnable mShowPrePageRunnable = new Runnable() {
        @Override
        public void run() {
            showPreOrNextPage(true);
        }
    };

    private final Runnable mShowNextPageRunnable = new Runnable() {
        @Override
        public void run() {
            showPreOrNextPage(false);
        }
    };

    private void showPreOrNextPage(boolean isPre){
        if(mPager == null || getCount() < 2){
            return ;
        }
        int position = mPager.getCurrentItem();
        int index = isPre ? position-1 : position+1;
        mPager.setCurrentItem(index);
    }

    private boolean getIsShakeSwipeEnable(){
        ContentResolver resolver = mContext.getContentResolver();
        int param = Settings.Global.getInt(resolver, Utils.QUICK_BROWSE, 0);
