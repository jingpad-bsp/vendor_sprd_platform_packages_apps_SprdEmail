...
public class EmlMessageViewFragment extends Fragment
    ...
    /* SPRD: Modify for bug496371 {@ */
    private MenuItem mPrintMenuItem;
    private boolean mMessageLoadedFinished;
    /* @} */

    /* SPRD: Modify for bug619950 {@ */
    private boolean isAlreadyBailOut;
    ...
    @Override
    public void onStart() {
        isAlreadyBailOut = false;
        final LoaderManager manager = getLoaderManager();
        if (manager != null && !isEmlFileExists(mEmlFileUri)) {
            manager.restartLoader(MESSAGE_LOADER, null, mMessageLoadCallbacks);
            manager.restartLoader(FILENAME_LOADER, null, mFilenameLoadCallbacks);
        }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        if (Utils.isRunningKitkatOrLater()) {
            ...
            /* SPRD: Modify for bug496371 {@ */
            mPrintMenuItem = menu.findItem(R.id.print_message);
            /* @} */
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        if (Utils.isRunningKitkatOrLater()) {
            if (mPrintMenuItem != null) {
                mPrintMenuItem.setEnabled(mMessageLoadedFinished && !mMessageLoadFailed);
            }

            /* SPRD: Modify for bug619950 {@ */
            if (mSettingsItem != null) {
                mSettingsItem.setEnabled(mMessageLoadedFinished && !mMessageLoadFailed);
            }
            /* @} */
        }
    }
    ...
    private class MessageLoadCallbacks
        ...
        public void onLoadFinished(Loader<ConversationMessage> loader, ConversationMessage data) {
            ...
            /* SPRD: Modify for bug496371 {@ */
            if (mPrintMenuItem != null) {
                mPrintMenuItem.setEnabled(true);
            }
            /* @} */

            /* SPRD: Modify for bug619950 {@ */
            if (mSettingsItem != null) {
                mSettingsItem.setEnabled(true);
            }
            /* @} */
        }
        ...
        public Loader<Cursor> onCreateLoader(int id, Bundle args) {
            switch (id) {
                case FILENAME_LOADER:
                    /* SPRD: Modify for bug623517 {@ */
                    return new EmlFilenameLoader(getActivity(), mEmlFileUri);
                    /* @} */
                default:
                    return null;
            }
    ...
    /*SPRD: modify for bug619950, origin bug616504 {@*/
    private boolean isEmlFileExists(Uri emlFileUri) {
        boolean isExists = false;
        InputStream in = null;

        if (emlFileUri != null) {
            try {
                in = getContext().getContentResolver().openInputStream(emlFileUri);
                if (null != in) {
                    isExists = true;
                }
            } catch (Exception e) {
                LogUtils.w(LOG_TAG, e, "isEmlFileExists");
            } finally {
                try {
                    if (null != in) {
                        in.close();
                    ...
                    LogUtils.w(LOG_TAG, e, "Failed to close emlfile stream");
                }
            }
        }
        ...
        if (!isExists) {
            ...
            bailOutOnLoadError();
            if (mPrintMenuItem != null) {
                mPrintMenuItem.setEnabled(false);
            }

            if (mSettingsItem != null) {
                mSettingsItem.setEnabled(false);
            }
        }
