...
public class SelectedConversationsActionMenu implements ActionMode.Callback,
    ...
    /*SPRD: Modify for bug641818 {@*/
    private static int mResId = -1;
    private static int mCheckedSetSize = -1;
    /* @}*/
    ...
    public SelectedConversationsActionMenu(
        ...
        /* SPRD modify for bug710205,bug709209{@ */
        mConversationCursorObserver = new DataSetObserver() {
            @Override
    ...
    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
        ...
        /* SPRD: Modify for 628241 selectall {@ */
            ...
            mUpdater.toggleCheckedStateAll(true);
        } else if (itemId == R.id.cancel_all) {
            mUpdater.toggleCheckedStateAll(false);
        }
        /* @} */
        else {
            handled = false;
    ...
    private void performDestructiveAction(final int action, UndoCallback undoCallback) {
        ...
        if (showDialog) {
            ...
            mResId = resId;
            mCheckedSetSize = conversations.size();
            final CharSequence message = Utils.formatPlural(mContext, resId, mCheckedSetSize);
    ...
    public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
        ...
        boolean isEmptySet = conversations.size() == 0;

        /* SPRD modify for bug709740{@ */
        if (isEmptySet) {
            final int size = menu.size();

            for (int i = 0; i < size; i++) {
                final MenuItem item = menu.getItem(i);
                item.setVisible(item.getItemId() == R.id.select_all);
            }
        ...
        final boolean canStar = mFolder != null && !mFolder.isTrash() && !mFolder.isDraft();
        final boolean canSupportRead = mFolder != null && !mFolder.isDraft();
        /* @} */
        ...
        /* SPRD: Modify for bug496342 {@ */
        read.setVisible(!showMarkUnread && canSupportRead);
        /* @} */
        final MenuItem unread = menu.findItem(R.id.unread);
        /* SPRD: Modify for bug496342 {@ */
        unread.setVisible(showMarkUnread && canSupportRead);
        /* @} */
        ...
        /* SPRD: Modify for 628241 selectall. bug709209, bug716588{@ */
        ConversationCursor conversationCursor = mUpdater.getConversationListCursor();
        int checkedSize = mCheckedSet.size();
        int cursorSize = conversationCursor == null ? 0 : conversationCursor.getNonoMostDeadCount();
        boolean showCancle = checkedSize >= cursorSize;
        final MenuItem selectAll = menu.findItem(R.id.select_all);
        if (selectAll != null) {
            selectAll.setVisible(!showCancle);
        }
        final MenuItem cancelAll = menu.findItem(R.id.cancel_all);
        if (cancelAll != null) {
            cancelAll.setVisible(showCancle);
        }
        /* @} */
    ...
    public void onSetEmpty() {
        ...
        /* SPRD: Modify for bug641818 {@ */
        ConfirmDialogFragment confirmDialogFragment = (ConfirmDialogFragment) mActivity
                .getFragmentManager().findFragmentByTag(ConfirmDialogFragment.DIALOG_TAG);
        // when confirmDialogFragment and CheckedSetSize is 0, dismiss the confirmDialogFragment
    ...
    private void updateCount() {
        if (mActionMode != null) {
            mActionMode.setTitle(String.format("%d", mCheckedSet.size()));
            updateSelection();
            /* SPRD: Modify for bug641818, 714026 {@ */
            // when confirmDialogFragment and mCheckedSetSize change, reset the message
            ConfirmDialogFragment confirmDialogFragment = (ConfirmDialogFragment) mActivity
                    .getFragmentManager().findFragmentByTag(ConfirmDialogFragment.DIALOG_TAG);
            if (confirmDialogFragment != null && mCheckedSetSize != mCheckedSet.size() && mResId != -1) {
                final CharSequence message = Utils.formatPlural(mContext, mResId,
                        mCheckedSet.size());
                AlertDialog alertDialog = (AlertDialog) confirmDialogFragment.getDialog();
                if (alertDialog != null) {
    ...
    public void activateWithoutStateCheck() {
        mListController.onCabModeEntered();
        mActivated = true;
        if (mActionMode == null) {
            mActivity.startSupportActionMode(this);
    ...
    private void destroy() {
        ...
        if (mUpdater != null) {
            mUpdater.refreshConversationList();
            mUpdater.unregisterConversationLoadedObserver(mConversationCursorObserver);//SPRD: Modify for bug710205
        }
        /* @}  */
