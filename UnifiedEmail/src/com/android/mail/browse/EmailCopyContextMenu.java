...
public class EmailCopyContextMenu implements OnCreateContextMenuListener{
    ...
    /* SPRD: Modify for bug646739 {@ */
    private class Compose implements MenuItem.OnMenuItemClickListener {
        private final Account account;
        private final CharSequence address;

        public Compose(Account account, CharSequence address) {
        ...
        @Override
        public boolean onMenuItemClick(MenuItem item) {
            ComposeActivity.composeToAddress(mContext, account, address.toString());
            //Handled & consumed event
            return true;
        }
    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo info) {
        ...
        if(!TextUtils.isEmpty(mAddress)) {
            ...
            menu.findItem(SEND_EMAIL_ITEM).setOnMenuItemClickListener(
                    new Compose(mAccount, mAddress));
