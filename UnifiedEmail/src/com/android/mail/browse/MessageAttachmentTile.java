...
public class MessageAttachmentTile extends AttachmentTile implements OnClickListener,
    ...
    private static final String SVG_IMAGE = "image/svg+xml"; //UNISOC: Modify for bug 1255449
    ...
    public void thumbnailLoadFailed() {
        ...
        /* UNISOC: Modify for bug1255449 {@ */
        if (mAttachment != null && SVG_IMAGE.equals(mAttachment.getContentType())
                && mAttachment.isPresentLocally()) {
            LogUtils.w(LOG_TAG, "svg image thumbnail load failed skip download it");
            return;
        }

        /* UNISOC: Modify for bug1152036 {@ */
        if (AttachmentUtils.canDownloadAttachment(getContext(), null)
                && (mAttachment != null && (!mAttachment.isDownloading()))) {
        /* @} */
            // Download if there is network. This check prevents the attachment
            // download from failing and making the error toast show
            LogUtils.w(LOG_TAG, "image thumbnail load failed start to download it");
