...
public class ConversationItemViewCoordinates {
    ...
    public static final class Config {
        ...
        private int mSnippetShowLines = MailPrefs.SnippetShowLinesValue.DEFAULT;
        ...
        public Config setDraft(boolean isDraft) {
            if (isDraft) {
                mGadgetMode = GADGET_NONE;
            }
            return this;
        }
        /* @} */
        ...
        public Config setSnippetShowLines(int lines) {
            mSnippetShowLines = lines;
            return this;
        }

        public int getSnippetShowLines() {
            return mSnippetShowLines;
        }
        ...
        public boolean isUnreadStateVisible() {
            return mShowUnreadState;
        }
        ...
        private int getCacheKey() {
            ...
            return Objects.hashCode(mWidth, mGadgetMode, mSnippetShowLines, mShowFolders,
                    mShowUnreadState, mShowReplyState, mShowPersonalIndicator, mLayoutDirection,
                    mUseFullMargins);
        }
    ...
    // Unread state of a conversation.
    final int unreadStateX;
    final int unreadStateXRight;
    final int unreadStateY;

    // Reply state of a conversation.
    final int replyStateX;
    final int replyStateXRight;
    final int replyStateY;

    final int personalIndicatorX;
    final int personalIndicatorXRight;
    ...
    private ConversationItemViewCoordinates(final Context context, final Config config,
        ...
        final int snippetShowLines = config.getSnippetShowLines();

        final int layoutId = (snippetShowLines > 0) ? R.layout.conversation_item_view
                : R.layout.conversation_item_view_no_snippet;
        ...
        final View unreadState = view.findViewById(R.id.unread_state);
        unreadState.setVisibility(config.isUnreadStateVisible() ? View.VISIBLE : View.GONE);
...
//        Utils.dumpViewTree((ViewGroup) view);
        ...
        if (config.isUnreadStateVisible()) {
            unreadStateX = getX(unreadState);
            unreadStateXRight = unreadState.getWidth();
            unreadStateY = getY(unreadState);
        } else {
            unreadStateX = unreadStateY = unreadStateXRight = 0;
        }

        if (config.isReplyStateVisible()) {
            replyStateX = getX(replyState) + (isRtl ? -unreadStateXRight : unreadStateXRight);
            replyStateXRight = replyState.getWidth();
            replyStateY = getY(replyState);
        } else {
            replyStateX = replyStateY = replyStateXRight = 0;
        }

        if (config.isPersonalIndicatorVisible()) {
            personalIndicatorX = getX(personalIndicator)
                    + (isRtl ? -(unreadStateXRight + replyStateXRight)
                            : unreadStateXRight + replyStateXRight);
            personalIndicatorXRight = personalIndicator.getWidth();
            personalIndicatorY = getY(personalIndicator);
        } else {
            personalIndicatorX = personalIndicatorY = personalIndicatorXRight = 0;
        }
        ...
        sendersX = getX(senders)
                + (isRtl ? -(unreadStateXRight + personalIndicatorXRight + replyStateXRight)
                        : unreadStateXRight + personalIndicatorXRight + replyStateXRight);
        ...
        sendersAscent = Math.round(senders.getPaint().ascent());
        ...
        if (snippetShowLines > 0) {
            ...
            snippetHeight = snippet.getHeight() * snippetShowLines;
        ...
        if (snippetShowLines > 0 && config.areFoldersVisible()) {
        ...
        height = view.getHeight() + sendersTopAdjust + getSnippetHeight(snippet, snippetShowLines);
        Utils.traceEndSection();
    }

    private static int getSnippetHeight(TextView snippet, int snippetShowLines) {
        if (snippetShowLines == 0) {
            return 0;
        }
        return snippet.getHeight() * (snippetShowLines - 1);
    }
