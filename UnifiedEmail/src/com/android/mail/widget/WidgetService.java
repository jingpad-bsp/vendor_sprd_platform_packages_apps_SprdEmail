...
public class WidgetService extends RemoteViewsService {
    ...
    public static void configureValidWidgetIntents(Context context, RemoteViews remoteViews,
        ...
        /* SPRD: Modify for bug582197 {@ */
        if (folderType != FolderType.DRAFT) {
            conversationIntent.setAction(Intent.ACTION_VIEW);
        } else {
            conversationIntent.setAction(ComposeActivity.ACTION_LAUNCH_COMPOSE);
        }
        /* @} */
        ...
        //LogUtils.e(LOG_TAG, "WidgetService configureValidWidgetIntents appWidgetId " + 	appWidgetId + " account " + account + " folderUri " + folderUri);
        //saveWidgetInformation(context, appWidgetId, account, folderUri.toString());
    }
    ...
    public static void saveWidgetInformation(Context context, int appWidgetId, Account account,
		...
		LogUtils.e(LOG_TAG, "WidgetService saveWidgetInformation appWidgetId " + 	appWidgetId + " account " + account + " folderUri " + folderUri);
    ...
    protected static class MailFactory
        ...
        public void onCreate() {
            LogUtils.e(LOG_TAG, "MailFactory onCreate mAppWidgetId " + mAppWidgetId + " mAccount " + mAccount + " mFolderUri " + mFolderUri.toString());
        ...
        public RemoteViews getViewAt(int position) {
            synchronized (sWidgetLock) {
                ...
                SendersView.format(mContext, conversation.conversationInfo, "",
                        MAX_SENDERS_LENGTH, senders, null, null, null, mAccount,
                ...
                // On click intent.
                /* SPRD: Modify for bug582197, bug598427, bug702763 {@ */
                if (mFolderType != FolderType.DRAFT) {
                    ...
                    try {
                        cursor = mContext.getContentResolver().query(
                                conversation.messageListUri, UIProvider.MESSAGE_PROJECTION, null,
                                null, null);
                        messageCursor = new MessageCursor(
                                new ObjectCursor<ConversationMessage>(cursor,
                                        ConversationMessage.FACTORY));
                        messageCursor.moveToFirst();
                        ConversationMessage message = messageCursor.getMessage();

                        final Intent composeIntent = new Intent(
                                ComposeActivity.ACTION_LAUNCH_COMPOSE);
                        composeIntent.putExtra("action", ComposeActivity.EDIT_DRAFT);
                        composeIntent.putExtra(Utils.EXTRA_ACCOUNT, mAccount.serialize());
                        if (message != null && message.uri != null) {
                            composeIntent.setData(Utils.normalizeUri(message.uri));
                        }
                        composeIntent.putExtra(ComposeActivity.EXTRA_FROM_EMAIL_TASK, true);
                        composeIntent.putExtra(ComposeActivity.ORIGINAL_DRAFT_MESSAGE, message);
                        ...
                        LogUtils.e(
                                LOG_TAG,
                                "appear CursorIndexOutOfBoundsException when loading draft"
                                        + e.toString());
                    } catch (SecurityException e) {
                        LogUtils.e(LOG_TAG,
                                "appear SecurityException when loading draft" + e.toString());
                    } finally {
        ...
        private SpannableStringBuilder elideParticipants(List<SpannableString> parts) {
            ...
            if (Folder.shouldShowRecipients(mFolderCapabilities)) {
                builder.append(SendersView.getFormattedToHeader(mContext));
                skipToHeader = true;
            }
        ...
        public void onLoadComplete(Loader<Cursor> loader, Cursor data) {
            ...
            if (loader == mFolderLoader) {
                ...
                /* UNISOC: Modify for bug1153175 {@ */
                if (!TextUtils.isEmpty(folderName) && mService.isAccountValid(mContext, mAccount)) {
                /* @} */
                    remoteViews.setViewVisibility(R.id.widget_folder, View.VISIBLE);
                    remoteViews.setViewVisibility(R.id.widget_compose, View.VISIBLE);
