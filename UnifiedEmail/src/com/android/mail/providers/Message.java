...
import com.android.mail.utils.Linkify;
...
public class Message implements Parcelable, HtmlMessage {
     ...
     */
    /**
     * @see UIProvider.MessageColumns#MAIL_PRIORITY
     */
    ...
    public Message(Cursor cursor) {
        if (cursor != null) {
             ...
             *@{
             */
            mailPriority = cursor.getInt(UIProvider.MESSAGE_MAIL_PRIORITY_COLUMN);
            /*@}*/
        }
    }
    ...
    public Message(Context context, MimeMessage mimeMessage, Uri emlFileUri)
        ...
        for (final Part viewablePart : viewables) {
            ...
            if (cids != null && cids.length == 1) {
                ...
                Attachment attachment = new Attachment(context, viewablePart, emlFileUri, messageId, cid,
                        true /* inline */);
                if (attachment.size != 0) {
                    mAttachments.add(attachment);
                } else {
                    final ContentResolver resolver = context.getContentResolver();
                    resolver.delete(attachment.uri, null, null);
                }
                /* @} */
            }
        }
        ...
        if (UIProvider.ENABLE_MAIL_PRIORITY_FEATURE) {
            if ("high".equalsIgnoreCase(mimeMessage.getImportance())
                    || "1".equalsIgnoreCase(mimeMessage.getMailPriority())) {
                mailPriority = UIProvider.MailPriority.HIGH;
                LogUtils.i(LogUtils.TAG, "MailPriority.HIGH");
            } else if ("low".equalsIgnoreCase(mimeMessage.getImportance())
                    || "5".equalsIgnoreCase(mimeMessage.getMailPriority())) {
                mailPriority = UIProvider.MailPriority.LOW;
                LogUtils.i(LogUtils.TAG, "MailPriority.LOW");
            } else {
                mailPriority = UIProvider.MailPriority.NORMAL;
            }
        }
        /*@}*/
    }
    ...
    public String getBodyAsHtml() {
        String body = "";
        /*SPRD:bug474616 distinguish hyperlink @{ */
        if (!TextUtils.isEmpty(bodyHtml)) {
            /* SPRD:bug520737 delete begin @{ */
            /*if (Utils.SUPPORT_SUPER_LINK) {
                body = Utils.getHtmlContext(bodyHtml);
            } else {
                body = bodyHtml;
            }*/
            /* @} */
