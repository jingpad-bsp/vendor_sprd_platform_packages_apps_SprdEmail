...
public final class OnePaneController extends AbstractActivityController {
    ...
    /* SPRD modify for bug712034 {@ */
    private static final int ILLEAGAL_ID = -2;
    /*@}*/
    ...
    protected ActionableToastBar findActionableToastBar(MailActivity activity) {
        ...
        tb.setFloatingActionButton(activity.findViewById(R.id.top_button));
        return tb;
    }
    ...
    public void showOrDismissTopButton(int mode, GoToTopCallback callback) {
        super.showOrDismissTopButton(mode, callback);
    }
    ...
    protected void showConversationList(ConversationListContext listContext) {
        ...
        if (!inInbox(mAccount, listContext)) {
            ...
            result = replaceFragment(conversationListFragment,
                    transition, TAG_CONVERSATION_LIST, R.id.content_pane);
            mLastConversationListTransactionId = result;
        } else {
            // If going to the inbox, clear the folder list transaction history.
            mInbox = listContext.folder;
            result = replaceFragment(conversationListFragment, transition, TAG_CONVERSATION_LIST,
                    R.id.content_pane);
    ...
    private int replaceFragment(Fragment fragment, int transition, String tag, int anchor) {
        ...
        try {
            ...
            id = fragmentTransaction.commitAllowingStateLoss();
            fm.executePendingTransactions();
        } catch (IllegalStateException e) {
            LogUtils.e(LOG_TAG, e, "Error while replaceFragment");
            id = ILLEAGAL_ID;
        }
    ...
    public boolean shouldShowSearchBarByDefault(int viewMode) {
        ...
        return (viewMode == ViewMode.SEARCH_RESULTS_LIST
                || viewMode == ViewMode.CONVERSATION_LIST)
