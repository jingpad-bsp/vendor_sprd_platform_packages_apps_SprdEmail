...
import com.android.mail.ui.FolderListFragment.LoadVipFolderMessageCountTask;
...
import com.sprd.mail.vip.utils.ThrottleContentObserver;
...
public class FolderListFragment extends ListFragment implements
    ...
    private ThrottleContentObserver mVipMemberObserver = null;
    ...
    public void onActivityCreated(Bundle savedState) {
        ...
        if (accountController != null) {
            ...
            mAllAccountsObserver = new AllAccountObserver(){
                ...
                public void onChanged(Account[] allAccounts) {
                    ...
                    mFolderWatcher.updateAccountList(FolderListFragment.this.getAllAccounts());  //UNISOC: Modify for bug1232943
                    rebuildAccountList();
                }
            };
        ...
        if (mVipMemberObserver == null) {
            mVipMemberObserver = new ThrottleContentObserver(new Handler(), activity,
                    new Runnable() {
                        @Override
                        public void run() {
                            // Execute in background thread, check if current
                            // account is available.
                            if (null == mCurrentAccount || null == mCurrentAccount.uri) {
                                return;
                            }
                            long accountId = (Long
                                    .parseLong(mCurrentAccount.uri.getLastPathSegment()));
                            new LoadVipFolderMessageCountTask(null, mActivity.getActivityContext())
                                    .executeSerial(accountId);
                        }
                    }, "VipMemberObserver");
            mVipMemberObserver.register(VipMember.UIPROVIDER_VIPMEMBER_NOTIFIER);
        }
    ...
    public void onDestroyView() {
        ...
        if (mVipMemberObserver != null) {
            mVipMemberObserver.unregister();
            mVipMemberObserver = null;
        }
        super.onDestroyView();
    ...
    protected void changeAccount(final Account account) {
        ...
        new LoadVipFolderMessageCountTask(null, mActivity.getActivityContext())
                .executeParallel(Long.parseLong(mNextAccount.uri.getLastPathSegment()));
        /* @} */
    ...
    public void onAccountSelected(Account account) {
        ...
        /* UNISOC: Modify for bug1113734 {@ */
        Folder defaultInbox = getDefaultInbox(account);
        if (defaultInbox == null) {
            Account currentAccount = AccountUtils.getAccount(mActivity.getActivityContext(), account.uri);
            if (currentAccount != null) account = currentAccount;
        }
        /* @}*/
    ...
    public void onLoadFinished(Loader<ObjectCursor<Folder>> loader, ObjectCursor<Folder> data) {
        if (mFolderAdapter != null) {
            if (loader.getId() == FOLDER_LIST_LOADER_ID) {
                ...
                new LoadVipFolderMessageCountTask(null, mActivity.getActivityContext())
                        .executeParallel(Long.parseLong(mCurrentAccount.uri.getLastPathSegment()));
    ...
    private interface FolderListFragmentCursorAdapter extends ListAdapter {
        ...
        void setVipFolderCount(int count);
    }
    ...
    private class FolderAdapter extends BaseAdapter implements FolderListFragmentCursorAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (type == DrawerItem.VIEW_FOLDER && item.mFolder.isType(UIProvider.FolderType.VIP)) {
                item.mFolder.unreadCount = mVipCount;
                LogUtils.d(LOG_TAG, "FolderListFragment.getView vip count : %d", item.mFolder.unreadCount);
            }
            /* SPRD: Modify for bug709933 @{ */
        ...
        @Override
        public void setVipFolderCount(int count) {
            mVipCount = count;
        }
    }
    ...
    private void setSelectedAccount(Account account) {
        ...
        if (changed) {
            ...
            AbstractActivityController controller = null;
            if (mActivity.getAccountController() instanceof AbstractActivityController) {
                controller = (AbstractActivityController) mActivity.getAccountController();
            }

            if (controller == null) {
                startLoaders();
            } else {
                controller.startFolderListLoaders();
            }
            /*@}*/
    ...
    class LoadVipFolderMessageCountTask extends EmailAsyncTask<Long, Void, Integer> {
        private Context mContext;
        public LoadVipFolderMessageCountTask(Tracker tracker, Context context) {
            super(tracker);
            mContext = context;
        }
        ...
        protected Integer doInBackground(Long... params) {
            return VipMemberCache.getVipMessagesCount(mContext, params[0], true);
        }

        @Override
        protected void onSuccess(Integer result) {
            updateVipMessageCount(result);
        }
    }

    public void updateVipMessageCount(int vipCount) {
        mFolderAdapter.setVipFolderCount(vipCount);
        mFolderAdapter.notifyDataSetChanged();
        LogUtils.d(LOG_TAG, "FolderListFragment.updateVipMessageCount count : %d", vipCount);
    }
}
