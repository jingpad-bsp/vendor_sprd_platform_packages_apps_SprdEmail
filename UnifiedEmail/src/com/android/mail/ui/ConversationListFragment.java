...
public final class ConversationListFragment extends Fragment implements
    ...
    public void onEndSwipe() {
        /* SPRD: Modify Bug494231, pop loading not dismiss. @{ */
        boolean isEnableRefresh = (!ConversationListContext.isSearchResult(mViewContext))
                && (mFolder != null && mFolder.refreshUri != null);
        mSwipeRefreshWidget.setEnabled(isEnableRefresh);
        /* @} */
    }
    ...
    public void onActivityCreated(Bundle savedState) {
        ...
        /* SPRD: Modify for bug733811 @{ */
        final List<ConversationSpecialItemView> specialItemViews =
                (helper != null && mAccount != null)
                ? ImmutableList.copyOf(
                        helper.makeConversationListSpecialViews(activity, mActivity, mAccount))
                : null;
        /* @} */
        ...
        final boolean showSearchHeader = ConversationListContext.isSearchResult(mViewContext)
                || ConversationListContext.isLocalSearchResult(mViewContext);
        /* @} */
        if (showSearchHeader) {
    ...
    public void onCreate(Bundle savedState) {
        ...
        mViewContext = ConversationListContext.forBundle(getContext(),
                args.getBundle(CONVERSATION_LIST_KEY));
    ...
    private void onListItemSelected(View view, int position) {
        /* UNISOC: Modify for bug1162057 {@ */
        if (view instanceof SwipeableConversationItemView) {
            LogUtils.d(LOG_TAG, "onListItemSelected view is SwipeableConversationItemView, return directly!");
        } else if (view instanceof ToggleableItem) {
        /* @} */
            /* SPRD: Modify for bug607188, bug709209 {@ */
            final boolean inCabMode = mActivity.getListHandler().isInCabMode();
            if (inCabMode) {
            /* @} */
                ((ToggleableItem) view).toggleCheckedState();
            } else {
                /* UNISOC: Modify for bug1230495 @{ */
//                if (inCabMode) {
//                    // this is a peek.
...
//                }
    ...
    public void onResume() {
        ...
        /* SPRD modify for bug702833 {@ */
        if (mCheckedSet != null) {
            mCheckedSet.addObserver(mConversationSetObserver);
        }
        /* @} */
        mCurrentViewHeight = Utils.getWindowHight(getActivity());
    }
    ...
    private void viewConversation(final int position) {
         ...
         */
            ...
            /* SPRD: Modify for bug497553 ,bug702763{@ */
            if (conv.isInDraft()) {
                if (isThreadFinished) {
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            try {
                                draftCursor = activity.getContentResolver().query(
                                        conv.messageListUri, UIProvider.MESSAGE_PROJECTION, null,
                                        null, null);
                                messageCursor = new MessageCursor(
                                        new ObjectCursor<ConversationMessage>(draftCursor,
                                                ConversationMessage.FACTORY));
                                messageCursor.moveToFirst();
                                ConversationMessage message = messageCursor.getMessage();
                                ComposeActivity.editDraft(activity, mAccount, message);
                            } catch (CursorIndexOutOfBoundsException e) {
                                LogUtils.e(LOG_TAG,
                                        "appear CursorIndexOutOfBoundsException when open draft"
                                                + e.toString());
                            } finally {
    ...
    private void setSwipeAction() {
        ...
        if (swipeSetting == Swipe.DISABLED
                ...
                /* SPRD: Modify for bug494800 {@ */
                || (mFolder != null && (mFolder.isTrash() || mFolder.isDraft()))) {
                /* @} */
    ...
    private void onCursorUpdated() {
        ...
        if (newCursor != null) {
            ...
            if (newCursorCount > 0) {
                /* SPRD:ADD for bug 495460 the database work will do in single thread. @{*/
                new Thread() {
                    @Override
                    public void run() {
        ...
        /* SPRD: Modify for bug497553 {@ */
        if (mCheckedSet != null && newCursor != null) {
            mCheckedSet.validateAgainstCursor(newCursor);
        }
        /* @} */
    }
        ...
        if (newCursor != null) {
            ...
            if (mInitialCursorLoading) {
                ...
                if (isCursorReadyToShow()) {
                    ...
                    /* SPRD modify for bug757808 {@ */
                    if (mActivity != null) {
                        FolderController folderController = mActivity.getFolderController();
                        Folder folder = folderController == null ? null : folderController
                                .getFolder();
    ...
    private boolean shouldShowTopButton() {
        ...
        return scrollY > mCurrentViewHeight;
    }
        ...
        if (rootView != null) {
            ...
            if (scrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE) {
                ...
                if (shouldShowTopButton()) {
                    mActivity.showOrDismissTopButton(View.VISIBLE, new GoToTopCallback() {
                        @Override
                ...
                mActivity.showOrDismissTopButton(View.GONE, null);
    ...
    /* SPRD:bug475886 add local search function @{ */
    public void updateSearchHeader(boolean showSearchHeader) {
        if (showSearchHeader && mSearchHeaderView != null) {
            return;
        }
        ...
        if (showSearchHeader) {
            ...
            mSearchResultCountTextView = null;
        }
    }
    /* @} */

    /* SPRD: Modify for 628241 selectall bug709209 {@ */
    public void toggleListViewCheckedStateAll(boolean check) {
        ConversationCheckedSet allSet = new ConversationCheckedSet();
        final ConversationCursor conversationCursor = getConversationListCursor();

        if (conversationCursor != null && conversationCursor.moveToFirst()) {
            do {
                allSet.add(conversationCursor.getConversation());
            } while (conversationCursor.moveToNext());
        }

        mListAdapter.setToggleState(check, allSet);
        mListAdapter.notifyDataSetChanged();
