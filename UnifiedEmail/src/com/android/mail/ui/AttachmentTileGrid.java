...
public class AttachmentTileGrid extends FrameLayout implements AttachmentPreviewCache,
    ...
    /* SPRD: Add for bug479199,limit the size of thumbnail in order to avoid out of memory){ */
    ...
    public void viewPhoto(MessageAttachmentTile source) {
        ...
        /* SPRD: Modify for bug498519 {@ */
        final String emailAddress = mAccount != null ? mAccount.getEmailAddress() : "";
        final String type = mAccount != null ? mAccount.getType() : "";
        MailPhotoViewActivity.startMailPhotoViewActivity(getContext(), emailAddress,
                type, mMessage, photoIndex);
        /* @} */
    }
    ...
    private void onMeasureForTiles(int widthMeasureSpec) {
        ...
        // but that the tiles can (and likely will) grow larger.
         ...
         /* SPRD: modify for bug479199,avoid out of memory){ */
    ...
    public Bitmap get(Attachment attachment) {
        final String attachmentIdentifier = attachment != null ? attachment.getIdentifierUri().toString() : null;// UNISOC: Modify for bug 1229277
        if (attachmentIdentifier != null) {
    ...
    /**
     *SPRD: Add for bug479199: Limit the size of grid tile to avoid out of memory according to the
