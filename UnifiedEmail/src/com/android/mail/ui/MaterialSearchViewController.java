...
public class MaterialSearchViewController implements ViewMode.ModeChangeListener,
    ...
    private boolean mPendingLocal = true;

    private int mPendingApproach = SearchParams.INVILID_SEARCH_APPROACH;
    /* @} */
    ...
    public MaterialSearchViewController(MailActivity activity, ActivityController controller,
        ...
        if (intent != null) {
            ...
            if (Intent.ACTION_SEARCH.equals(intent.getAction())) {
                boolean isLocal = intent.getBooleanExtra(ConversationListContext.EXTRA_SEARCH_IS_LOCAL, false);
                mSearchActionView.setApprochViewVisibility(isLocal);
                if (isLocal) {
                    mSearchActionView.setApproach(intent.getIntExtra(ConversationListContext.EXTRA_SEARCH_APPROACH,
    ...
    public void onViewModeChanged(int newMode) {
        ...
        mViewMode = newMode;
        ConversationListContext lisContext = mController.getCurrentListContext();
        if (newMode == ViewMode.CONVERSATION && oldMode == ViewMode.CONVERSATION_LIST
                && ConversationListContext.isLocalSearchResult(lisContext)) {
            mPendingQuery = lisContext.searchQuery;
            mPendingApproach = lisContext.searchApproach;
            mPendingLocal = lisContext.isLocalSearch;
        }
    ...
    public String getPendingQuery() {
        return mPendingQuery;
    }

    public void setPendingQuery(String mPendingQuery) {
        this.mPendingQuery = mPendingQuery;
    }

    public int getPendingApproach() {
        return mPendingApproach;
    }

    public void setPendingApproach(int mPendingApproach) {
        this.mPendingApproach = mPendingApproach;
    }

    public boolean isPendingLocal() {
        return mPendingLocal;
    }

    public void setPendingLocal(boolean mPendingLocal) {
        this.mPendingLocal = mPendingLocal;
    }
    /* @} */
    ...
    public boolean handleBackPress() {
        ...
        if (shouldShowSearchBar && mSearchSuggestionList.isShown()) {
            ...
            return true;
        } else if (Utils.SUPPORT_LOCAL_SEARCH && mSearchActionView.isShown()
                && ConversationListContext.isLocalSearchResult(lisContext)) {
            showSearchActionBar(SEARCH_VIEW_STATE_GONE);
            return true;
        }
    ...
    public void showSearchActionBar(int state, boolean animate) {
        ...
        /* UNISOC: Modify for Bug1077950 {@ */
        final int suggestionListVisibility = isStateVisible && mSearchSuggestionList.getCount() > 0 ? View.VISIBLE : View.GONE;
        /* @} */
        if (animate) {
    ...
    public void onQueryTextChanged(String query) {
        ...
        if (Utils.SUPPORT_LOCAL_SEARCH && ViewMode.CONVERSATION_LIST == mViewMode) {
            android.os.Message msg = mHandler.obtainMessage(MSG_REQUEST_SEARCH, 0, 0, query);
            mHandler.removeMessages(msg.what);
            mHandler.sendMessageDelayed(msg, REQUEST_SEARCH_DELAY);
        }
    }
    /* @} */
    ...
    public void onSearchPerformed(String query) {
        if(mSearchActionView != null) {
            onSearchPerformed(query, mSearchActionView.getSearchApproach());
        }
    }

    private void onSearchPerformed(String query, int approach) {
        query = query.trim();
        if (Utils.SUPPORT_LOCAL_SEARCH) {
            mController.executeLocalSearch(query, approach);
        } else if(!TextUtils.isEmpty(query)) {
            mSearchActionView.clearSearchQuery();
            mController.executeSearch(query, approach);
        }
    }
    /* @} */
    ...
    /* SPRD:bug475886 add local search function @{ */
    private static final int MSG_REQUEST_SEARCH = 0;
    private static final int REQUEST_SEARCH_DELAY = 300;
