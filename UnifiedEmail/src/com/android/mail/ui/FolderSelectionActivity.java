...
public class FolderSelectionActivity extends AppCompatActivity implements OnClickListener,
    ...
    private void doCancel() {
        ...
        LogUtils.wtf(LOG_TAG, "FolderSelectionActivity onFolderChanged doCancel() finish");
        finish();
    }
    ...
    protected void createWidget(int id, Account account, Folder selectedFolder) {
        ...
        LogUtils.wtf(LOG_TAG, "FolderSelectionActivity createWidget finish");
        finish();
        //System.exit(0);
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == DialogInterface.BUTTON_POSITIVE) {
            ...
            LogUtils.wtf(LOG_TAG, "FolderSelectionActivity onClick createWidget finish");
        ...
        if (!folder.equals(mSelectedFolder)) {
            ...
            if (mConfigureShortcut) {
                ...
                shortcutNameIntent.putExtra(ShortcutNameActivity.EXTRA_SHORTCUT_NAME,
                        mSelectedFolder.name);
                /* @} */

                startActivity(shortcutNameIntent);
                LogUtils.wtf(LOG_TAG, "FolderSelectionActivity onFolderChanged finish");
                finish();
            } else if (mConfigureWidget) {
				LogUtils.wtf(LOG_TAG, "FolderSelectionActivity onFolderChanged createWidget finish");
                createWidget(mAppWidgetId, mAccount, mSelectedFolder);
            }
        }
    }
