...
public class ClearPictureApprovalsDialogFragment extends DialogFragment implements OnClickListener {
    ...
    public void onClick(DialogInterface dialog, int which) {
        /* SPRD: Modify for 648771 {@ */
        Context context = getActivity();
        if (context != null && which == DialogInterface.BUTTON_POSITIVE) {
            final MailPrefs mailPrefs = MailPrefs.get(context);
            mailPrefs.clearSenderWhiteList();
            Toast.makeText(context, R.string.sender_whitelist_cleared, Toast.LENGTH_SHORT)
                    .show();
        }
        /* @} */
    }
