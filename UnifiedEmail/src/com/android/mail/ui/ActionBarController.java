...
public class ActionBarController implements ViewMode.ModeChangeListener {
    ...
    private MenuItem mSortOrderItem;
    private MenuItem mSearchOrderItem;
    ...
    private static final class UpdateProvider extends AsyncTask<Bundle, Void, Void> {
        final Uri mAccount;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (ViewMode.isListMode(getMode())) {
            mSortOrderItem = menu.findItem(R.id.change_sorting_method);
            mSearchOrderItem = menu.findItem(R.id.search);
        }
    ...
    public void validateVolatileMenuOptionVisibility() {
        boolean shouldHideItems = mController.shouldHideMenuItems();
        Utils.setMenuItemPresent(mEmptyTrashItem, !shouldHideItems&&mAccount != null && mFolder != null
        ...
        Utils.setMenuItemPresent(mEmptySpamItem, !shouldHideItems&& mAccount != null && mFolder != null
                && mAccount.supportsCapability(AccountCapabilities.EMPTY_SPAM)
                && mFolder.isType(FolderType.SPAM) && mFolder.totalCount > 0
                && (mController.getConversationListCursor() == null
                || mController.getConversationListCursor().getCount() > 0));

        boolean isEmptyList = ViewMode.isListMode(getMode())
                && mController.getConversationCount() == 0;
        /* SPRD: Modify for bug 732463 @{ */
        boolean isEmptyDraftCount = mFolder != null && mFolder.isDraft()
                && mFolder.totalCount == 0;
        Utils.setMenuItemPresent(mSortOrderItem,
                !shouldHideItems && !isEmptyList && !(mFolder != null && mFolder.isOutbox()) && !isEmptyDraftCount);

        if (Utils.SUPPORT_LOCAL_SEARCH) {
            Utils.setMenuItemPresent(mSearchOrderItem,
                    !isEmptyList && mAccount != null && mAccount.supportsSearch() && !mIsOnTablet && !isEmptyDraftCount);
        } else {
            Utils.setMenuItemPresent(mSearchOrderItem,
                    !isEmptyList && mAccount != null && mAccount.supportsRemoteSearch()
                            && !mIsOnTablet && !isEmptyDraftCount);
        }
        /* @} */
    }
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        int mode = getMode();
        boolean shouldHideMenuItems = mController.shouldHideMenuItems()
                || (ViewMode.isListMode(mode) && mController.getConversationCount() == 0);
        ...
        switch (mode) {
            ...
            case ViewMode.SEARCH_RESULTS_LIST:
                ...
                if(Utils.SUPPORT_LOCAL_SEARCH) {
                    ...
                    Utils.setMenuItemPresent(menu, R.id.search,
                            mAccount.supportsRemoteSearch() && !mIsOnTablet);
                }
                /* @} */
                Utils.setMenuItemPresent(menu, R.id.change_sorting_method,
                        !(mFolder != null && mFolder.isOutbox()));
        }
    ...
    private void setSubtitle(String title) {
        if (!TextUtils.equals(title, mActionBar.getSubtitle())) {
    ...
    private void setFolderAndAccount() {
        ...
        setSubtitle(mAccount.getEmailAddress());
    }
