...
public class ConversationViewFragment extends AbstractConversationViewFragment implements
    ...
    public void onUserVisibleHintChanged() {
        ...
        if (!userVisible && mProgressController != null) {
    ...
    protected void loadContent() {
        /* SPRD: Modify for bug611762 {@ */
        final LoaderManager manager = getLoaderManager();
        if (manager.getLoader(MESSAGE_LOADER) != null) {
            manager.restartLoader(MESSAGE_LOADER, Bundle.EMPTY, getMessageLoaderCallbacks());
        } else {
            manager.initLoader(MESSAGE_LOADER, Bundle.EMPTY, getMessageLoaderCallbacks());
        }
        /* @} */
    }
    ...
    private MessageHeaderItem getLastMessageHeaderItem() {
        /* SPRD:bug628647 modify for stared email crash issue @{ */
        int pos = mAdapter == null ? 0 : mAdapter.getCount();
        /* @} */
        while (--pos >= 0) {
    ...
    public void onConversationViewHeaderStarredChange(boolean starred){
        if (getLastMessageHeaderItem() != null) {
            getLastMessageHeaderItem().updateStarredStatus(starred);
        }
    }
    /* @} */
    ...
    public void showUntransformedConversation() {
        ...
        /* SPRD: Modify for bug668282 @{ */
        if (cursor != null && isResumed()) {
    ...
    public ConversationMessage getMessageForClickedUrl(String url) {
        /* SPRD: Modify for bug499079, bug668634 {@ */
        ...
        if (domMessageId == null) {
            if (messageCursor.moveToFirst()) {
                return messageCursor.getMessage();
            }
            return null;
        }
        /* @} */
