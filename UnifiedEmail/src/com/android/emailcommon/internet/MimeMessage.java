...
public class MimeMessage extends Message {
    ...
    private Date mReceivedDate;//UNISOC: Modify for bug1441107
    private Body mBody;
    protected int mSize;
    ...
    // localization code.
    private static final String DATE_FORMAT = "EEE, dd MMM yyyy HH:mm:ss Z";//UNISOC:Modify for bug1234482
    ...
    private MimeStreamParser init() {
        ...
        mReplyTo = null;
        mSentDate = null;
        mReceivedDate = null;//UNISOC: Modify for bug1441107
        mBody = null;
    ...
    public void parse(InputStream in, EOLConvertingInputStream.Callback callback)
            throws IOException, MessagingException, MimeException {
        final MimeStreamParser parser = init();
    ...
    public Date getReceivedDate() throws MessagingException {
        ...
        if (mReceivedDate == null) {
            String receivedInfo = getFirstHeader("Received");
            if (receivedInfo == null) {
                return null;
            }
            String[] parts = MimeUtility.unfold(receivedInfo).split("\\;");
            if (parts.length > 1) {
                String receivedDateString = parts[1].trim();
                try {
                    DateTimeField field = (DateTimeField)DefaultFieldParser.parse("Date: "
                            + MimeUtility.unfoldAndDecode(receivedDateString));
                    mReceivedDate = field.getDate();
                    ...
                    LogUtils.v(LogUtils.TAG, "Message missing Received header");
                }
    ...
    public void setSentDate(Date sentDate) throws MessagingException {
        setHeader("Date", new SimpleDateFormat(DATE_FORMAT, Locale.US).format(sentDate));//UNISOC:Modify for bug1234482
    ...
    public String getImportance() {
        String ret = null;
        try {
            ret = getFirstHeader("Importance");
        } catch (MessagingException e) {
            ret = null;
            LogUtils.e(LogUtils.TAG, "get Importance fail: %s", e.getMessage());
        }
        return ret;
    }
    ...
    public String getMailPriority() {
        String ret = null;
        try {
            ret = getFirstHeader("X-Priority");
        } catch (MessagingException e) {
            ret = null;
            LogUtils.e(LogUtils.TAG, "get X-Priority fail: %s", e.getMessage());
        }
        return ret;
    }
    /*@}*/
    ...
    public void setBody(Body body) throws MessagingException {
        ...
        if (body instanceof Multipart) {
            ...
            setHeader(MimeHeader.HEADER_CONTENT_TYPE, String.format("%s;%n charset=utf-8",
                    getMimeType()));  //UNISOC: Modify for bug1232943
