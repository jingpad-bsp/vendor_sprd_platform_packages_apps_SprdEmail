...
public class Address implements Parcelable {
    ...
    public static String getFirstMailAddress(String packedString) {
        ...
        int addressEndIndex = packedString.indexOf(LIST_DELIMITER_PERSONAL);
        ...
        int pairEndIndex = packedString.indexOf(LIST_DELIMITER_EMAIL);
        if (pairEndIndex != -1) {
    ...
    public static synchronized Address getEmailAddress(String rawAddress) {
        ...
        if (tokens.length > 0) {
            ...
            name = "";
            address = Html.fromHtml(rawAddress).toString();//UNISOC: Modify for bug 1229277
        }
    ...
    public void setPersonal(String personal, boolean isNeedDecode) {
        if (personal != null) {
            ...
            if (isNeedDecode) {
                personal = DecoderUtil.decodeEncodedWords(personal, DecodeMonitor.STRICT); // UNISOC: Modify for bug 1206799
            }
     ...
     * @param isNeedDecode SPRD: indicated that we should force decoding each address string
            ...
            if (!TextUtils.isEmpty(address)) {
                if (isValidAddress(address)) {
                    ...
                    if (TextUtils.isEmpty(name)) {
                        /// SPRD: change for one special case:
