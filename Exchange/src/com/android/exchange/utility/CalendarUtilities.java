...
public class CalendarUtilities {
    ...
    static TimeZoneDate getTimeZoneDateFromSystemTime(byte[] bytes, int offset) {
        ...
        // MSFT months are 1 based; TimeZone is 0 based
        int num = getWord(bytes, offset + MSFT_SYSTEMTIME_MONTH);
        if (num == 0) {
            return null;
        } else {
            tzd.month = num -1;
    ...
    static TimeZone tziStringToTimeZoneImpl(String timeZoneString, int precision) {
        ...
        if (zoneIds.length > 0) {
            ...
            if (dstEnd == null) {
                ...
                long dstSavings =
                    (-1 * ((long) getLong(timeZoneBytes, MSFT_TIME_ZONE_DAYLIGHT_BIAS_OFFSET))) * MINUTES;//UNISOC: Modify for bug 1234479
                ...
                if (dstStart != null && (dstStart.hour != dstEnd.hour) && (precision == STANDARD_DST_PRECISION)) {
