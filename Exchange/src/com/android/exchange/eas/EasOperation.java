...
public abstract class EasOperation {
     ...
     */
    public boolean init() {
        /* SPRD: Modify for bug498577 ,628975{@ */
        boolean result = true;
        try{
            if (mAccount != null && mContext != null &&
                    (mAccount.mSyncKey == null || "0".equals(mAccount.mSyncKey))) {
                Account tempAccount = Account.restoreAccountWithId(mContext, getAccountId());
                if (tempAccount != null) {
                    mAccount.mSyncKey = tempAccount.mSyncKey;
                }
            }
        }catch (Exception e) {
            LogUtils.w(LOG_TAG, "Exception while restoreAccountWithId", e);
            result = false;
        } finally {
           return  result  ;
        }
    ...
    protected static void expandedAddDeviceInformationToSerializer(final Serializer s,
        ...
        if (tm != null) {
            try {
                ...
                LogUtils.d(LOG_TAG, e,"SecurityException in get phone state ");
            }
        }
    ...
    protected static void requestSyncForMailbox(final android.accounts.Account amAccount,
        ...
        LogUtils.d(LOG_TAG, "requestSync EasOperation requestSyncForMailbox %s, %s",
    ...
    protected static void requestSyncForMailboxes(final android.accounts.Account amAccount,
        ...
        LogUtils.d(LOG_TAG, "EasOperation requestSyncForMailboxes  %s, %s",
                amAccount.toString(), extras.toString());
    }

    public static int translateSyncResultToUiResult(final int result) {
        LogUtils.d(LOG_TAG, "translateSyncResultToUiResult,result %d", result);
        switch (result) {
