...
public class EasFolderSync extends EasOperation {
    ...
    private static LongSparseArray<Object> sFolderSyncLocks = new LongSparseArray();
    private static LongSparseArray<Boolean> sIsInvalidStatusKeyList = new LongSparseArray();
    /* @} */
    ...
    public int performOperation() {
        if (mStatusOnly) {
            ...
            return performOperationSynchronized(); //UNISOC: Modify for bug 1167616
        }
    }

    /* UNISOC: Modify for bug1167616 {@ */
    private int performOperationSynchronized() {
        int result;
        Object lock = getFolderSyncLock(getAccountId());
        synchronized (lock) {
            result = super.performOperation();
            if (mAccount != null && getIsInvalidStatusKey(getAccountId())) {
                resetAccountSyncKey();
                result = super.performOperation();
            }
        }
    ...
    private static synchronized Object getFolderSyncLock(long id) {
        Object lock;
        synchronized (EasFolderSync.class) {
            lock = sFolderSyncLocks.get(id);
            if (lock == null) {
                lock = new Object();
                sFolderSyncLocks.put(id, lock);
            }
        }
        return lock;
    }
    ...
    /* UNISOC: Modify for bug1167616 {@ */
    public static void setIsInvalidStatusKey(long accountId, boolean flag) {
        sIsInvalidStatusKeyList.put(accountId, Boolean.valueOf(flag));
    }

    public static boolean getIsInvalidStatusKey(long accountId) {
        Boolean isInvalidStatusKey = (Boolean) sIsInvalidStatusKeyList.get(accountId);
        if (isInvalidStatusKey != null) {
            return isInvalidStatusKey.booleanValue();
        }
        return false;
    }

    private void resetAccountSyncKey() {
        LogUtils.d(LOG_TAG, "resetAccountSyncKey for account " + mAccount.mId);
        mAccount.mSyncKey = "0";
        ContentValues cv = new ContentValues();
        cv.put(AccountColumns.SYNC_KEY, mAccount.mSyncKey);
        mContext.getContentResolver().update(ContentUris.withAppendedId(Account.CONTENT_URI,
