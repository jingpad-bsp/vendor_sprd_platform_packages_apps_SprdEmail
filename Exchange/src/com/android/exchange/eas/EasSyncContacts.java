...
import com.android.exchange.EasPermissionRequestor;
...
public class EasSyncContacts extends EasSyncCollectionTypeBase {
    ...
    /* SPRD: modify for bug746809{@ */
    private static final String FILE_AS_DEFAULT_VALUE = "Contacts";
    /* @} */
    ...
    static {
        ...
        /* UNISOC: Modify for bug1234482 @{ */
        // Time format documented at http://msdn.microsoft.com/en-us/library/ee201818(v=exchg.80).aspx
        final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.S'Z'", Locale.US);
        DATE_FORMAT.setTimeZone(TimeZone.getTimeZone("UTC"));
        DATE_FORMATS = new DateFormat[] { DATE_FORMAT, SHORT_DATE_FORMAT };
        /* @} */
    }
                ...
                for (final DateFormat format : DATE_FORMATS) {
                    try {
                        ...
                        if (date != null) {
                            ...
                            final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.S'Z'", Locale.US);
                            DATE_FORMAT.setTimeZone(TimeZone.getTimeZone("UTC"));
                            s.data(tag, DATE_FORMAT.format(date));
                            /* @} */
                            return;
                        }
        ...
        try {
            ...
            while (ei.hasNext()) {
                ...
                if (!hasSetFileAs) {
                    ...
                    /*
                     * SPRD: modify for bug746809 Eas upload contacts issue. Since fileAs can not be
                     * omited, if it is still null after computed according names and email address
                     * of the very contact assing a default value to it. {@
                     */
                    if (TextUtils.isEmpty(fileAs)) {
                        fileAs = FILE_AS_DEFAULT_VALUE;
                    }
                    /* @} */
    ...
    public static void wipeAccountFromContentProvider(final Context context,
        ...
        try {
            context.getContentResolver().delete(uriWithAccountAndIsSyncAdapter(
                    /* SPRD: Modify for bug498577 {@ */
                    ContactsContract.RawContacts.CONTENT_URI, emailAddress), ContactsContract.RawContacts.DIRTY + "<>?", new String[] {"1"});
                    /* @} */
        } catch (IllegalArgumentException e) {
            LogUtils.e(TAG, "ContactsProvider disabled; unable to wipe account.");
        /* UNISOC: Modify for bug1155628 {@ */
        } catch (SecurityException e) {
            LogUtils.e(TAG, e, "wipe ContactsProvider permission denial.");
            Intent intent = new Intent(context, EasPermissionRequestor.class);
