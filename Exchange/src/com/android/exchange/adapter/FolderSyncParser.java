...
public class FolderSyncParser extends AbstractSyncParser {
    ...
    private List<MailboxEx> mMailboxExListForLocalDB = new ArrayList(); //UNISOC: Modify for bug 1167616
    ...
    /* UNISOC: Modify for bug1167616 {@ */
    private List<MailboxEx> getMailboxesExByAccountId(long accountId) {
        List<MailboxEx> mailboxExList = new ArrayList();
        List<Mailbox> mailboxList = Mailbox.getMailboxesByAccountId(mContext, accountId);
        LogUtils.i(TAG, "getMailboxesExByAccountId accountId " + accountId + " mailboxList " + mailboxList.size());
        if (mailboxList.size() == 0) {
            return mailboxExList;
        }
        for (int i = 0; i < mailboxList.size(); i++) {
            Mailbox mailbox = (Mailbox) mailboxList.get(i);
            if (mailbox == null) {
                LogUtils.i(TAG, "getMailboxesExByAccountId mailbox is null continue");
            } else if (mailbox.mType == Mailbox.TYPE_SEARCH) {
                LogUtils.i(TAG, "getMailboxesExByAccountId mailbox is search type continue");
            } else if (TextUtils.isEmpty(mailbox.mServerId)) {
                LogUtils.i(TAG, "getMailboxesExByAccountId mailbox.mServerId " + mailbox.mServerId + " is empty continue");
            } else {
                mailboxExList.add(new MailboxEx(mailbox));
            }
        }
        return mailboxExList;
    }

    private boolean processMailboxChangedIfNeeded(String serverId, ContentValues cv) {
        int i = 0;
        while (i < mMailboxExListForLocalDB.size()) {
            MailboxEx mailboxEx = (MailboxEx) this.mMailboxExListForLocalDB.get(i);
            Mailbox mailbox = mailboxEx.getMailbox();
            if (mailbox == null || TextUtils.isEmpty(serverId) || !serverId.equals(mailbox.mServerId)) {
                i++;
            } else {
                mailboxEx.setIsExistInServer(true);
                mCreatedFolderTypes.put(mailbox.mType, true);
                String parentServerIdFromCv = cv.getAsString(MailboxColumns.PARENT_KEY);
                String displayNameFromCv = cv.getAsString(MailboxColumns.DISPLAY_NAME);
                boolean parentServerIdChanged = (TextUtils.isEmpty(mailbox.mParentServerId)
                        || mailbox.mParentServerId.equals(parentServerIdFromCv)) ? false : true;
                boolean displayNameChanged = (TextUtils.isEmpty(mailbox.mDisplayName)
                        || mailbox.mDisplayName.equals(displayNameFromCv)) ? false : true;
                if (parentServerIdChanged || displayNameChanged) {
                    LogUtils.i(TAG, "processMailboxChangedIfNeeded mParentServerId or mDisplayName changed");
                    ContentValues cvUpdate = new ContentValues(2);
                    if (parentServerIdChanged) {
                        cvUpdate.put(MailboxColumns.PARENT_KEY, parentServerIdFromCv);
                        if (!TextUtils.isEmpty(mailbox.mParentServerId)) {
                            mParentFixupsNeeded.add(mailbox.mParentServerId);
                        }
                        if (!TextUtils.isEmpty(parentServerIdFromCv)) {
                            mParentFixupsNeeded.add(parentServerIdFromCv);
                        }
                    }
                    if (displayNameChanged) {
                        cvUpdate.put(MailboxColumns.DISPLAY_NAME, displayNameFromCv);
                    }
                    mOperations.add(ContentProviderOperation.newUpdate(ContentUris.withAppendedId(Mailbox.CONTENT_URI, mailbox.mId)).withValues(cvUpdate).build());
                } else {
                    LogUtils.i(TAG, "processMailboxChangedIfNeeded mParentServerId or mDisplayName not changed");
                }
                return true;
            }
        }
    ...
    private void processOrphanMailboxInLocalDB() {
        for (int i = 0; i < mMailboxExListForLocalDB.size(); i++) {
            MailboxEx mailboxEx = (MailboxEx) mMailboxExListForLocalDB.get(i);
            Mailbox mailbox = mailboxEx.getMailbox();
            if (mailbox != null && !mailboxEx.getIsExistInServer()) {
                LogUtils.i(TAG, "processOrphanMailboxInLocalDB mailbox does not exist in server delete it " + mailbox);
                if (!TextUtils.isEmpty(mailbox.mParentServerId)) {
                    mParentFixupsNeeded.add(mailbox.mParentServerId);
                }
                mOperations.add(ContentProviderOperation.newDelete(ContentUris.withAppendedId(Mailbox.CONTENT_URI, mailbox.mId)).build());
                AttachmentUtilities.deleteAllMailboxAttachmentFiles(mContext, mAccountId, mailbox.mId);
            }
        }
    }
    /* @} */
    ...
    public boolean parse() throws IOException, CommandStatusException {
        int status;
        int orginalStatus; //UNISOC: Modify for bug 1167616
        boolean res = false;
        ...
        if (mInitialSync) {
            ...
            /* UNISOC: Modify for bug1167616 {@ */
            if (EasFolderSync.getIsInvalidStatusKey(mAccount.mId)) {
                LogUtils.i(TAG, "Restore from status=9(FOLDER_STATUS_INVALID_KEY), won't delete mailbox here for account " + mAccountId);
                EasFolderSync.setIsInvalidStatusKey(mAccount.mId, false);
                mMailboxExListForLocalDB = getMailboxesExByAccountId(mAccount.mId);
            } else {
                wipe();
            }
            /* @} */
        ...
        while (nextTag(START_DOCUMENT) != END_DOCUMENT) {
            if (tag == Tags.FOLDER_STATUS) {
                status = getValueInt();
                orginalStatus = status; //UNISOC: Modify for bug 1167616
                /* SPRD modify for bug708420{@ */
                LogUtils.i(TAG, "FolderSyncParser parse FOLDER_STATUS status:" + status);
                /* @} */
                ...
                /* SPRD modify for bug708420{@ */
                LogUtils.i(TAG, "Duplicate mailboxes found for account %d: %d", mAccountId,
                        dupes);
                /* @} */
                if (dupes > 0) {
                ...
                if (status != Eas.FOLDER_STATUS_OK) {
                    ...
                    /* SPRD: Modify for bug 522743, bug 708420{@ */
                    } else if (status == Eas.FOLDER_STATUS_INVALID_KEY
                            || CommandStatus.isBadSyncKey(status)) {
                    /* @} */
                        /* UNISOC: Modify for bug1167616 {@ */
                        if (orginalStatus == Eas.FOLDER_STATUS_INVALID_KEY) {
                            LogUtils.i(TAG, "FOLDER_STATUS_INVALID_KEY won't wipe");
                            EasFolderSync.setIsInvalidStatusKey(mAccount.mId, true);
                            return false;
                        }
                        /* @} */
                        LogUtils.i(TAG, "FolderSyncParser FOLDER_STATUS_INVALID_KEY wipe data");
                        wipe();
                        // Reconstruct _main
                        res = true;
                        resetFolders = true;
        ...
        /* SPRD modify for bug708420{@ */
        if (resetFolders) {
            LogUtils.i(TAG, "FolderSyncParser Folders Rested issue a new sync");
    ...
    /* UNISOC: Modify for bug1167616 {@ */
    private static class MailboxEx {
        ...
        public boolean getIsExistInServer() {
            return mIsExistInServer;
        }

        public void setIsExistInServer(boolean flag) {
            mIsExistInServer = flag;
        }
    }
    /* @} */
    ...
    void restoreMailboxSyncOptions() {
        try {
            ...
            mBindArguments[1] = mAccountIdAsString;
            /* UNISOC: Modify for bug1233291 {@ */
            for (Entry<String, SyncOptions> tmpEntry : mSyncOptionsMap.entrySet()) {
                SyncOptions options = tmpEntry.getValue();
                cv.put(MailboxColumns.SYNC_INTERVAL, options.mInterval);
                cv.put(MailboxColumns.SYNC_LOOKBACK, options.mLookback);
                mBindArguments[0] = tmpEntry.getKey();
    ...
    /* UNISOC: Modify for bug1167616 {@ */
    private ContentValues constructContentValues(final String name, final String serverId,
    ...
    private void addMailboxOp(final String name, final String serverId,
            ...
            throws IOException {
        mOperations.add(
                ContentProviderOperation.newInsert(Mailbox.CONTENT_URI).withValues(
                        constructContentValues(name, serverId, parentServerId, mailboxType, fromServer)).build());
    ...
    private void addParser() throws IOException {
        ...
        if (name != null && serverId != null && parentId != null) {
            ...
            if (mailboxType != Mailbox.TYPE_NONE) {
                ...
                /* UNISOC: Modify for bug1167616 {@ */
                ContentValues cv = constructContentValues(name, serverId, parentId, mailboxType, true);
                if (mInitialSync && processMailboxChangedIfNeeded(serverId, cv)) {
                    LogUtils.i(TAG, "addParser mailbox " + serverId + " is exist only update it");
                    return;
                }
                /* @} */
    ...
    public void commit() throws IOException {
        ...
        if (mInitialSync) {
            ...
            processOrphanMailboxInLocalDB(); //UNISOC: Modify for bug 1167616
        }
