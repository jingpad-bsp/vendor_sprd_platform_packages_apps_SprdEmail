...
public class EmailSyncParser extends AbstractSyncParser {
        ...
        while (nextTag(endingTag) != END) {
            switch (tag) {
                ...
                case Tags.BASE_ATTACHMENTS: // BASE_ATTACHMENTS is used in EAS 12.0 and up
                    ...
                    break;
                case Tags.EMAIL_TO:
                    Address[] to = Address.parse(getValue());
                    if (mMailbox.mType == Mailbox.TYPE_SENT
                            || mMailbox.mType == Mailbox.TYPE_OUTBOX
                            || mMailbox.mType == Mailbox.TYPE_DRAFTS) {
                        msg.mDisplayName = (to != null && to.length > 0) ? to[0]
                                .toFriendly() : "";
                    }
                    msg.mTo = Address.toString(to);
                    break;
                case Tags.EMAIL_FROM:
                    ...
                    if (!(mMailbox.mType == Mailbox.TYPE_SENT
                            || mMailbox.mType == Mailbox.TYPE_OUTBOX
                            || mMailbox.mType == Mailbox.TYPE_DRAFTS)) {
                        msg.mDisplayName = (froms != null && froms.length > 0) ? froms[0]
                                .toFriendly() : "";
                    }
                 ...
                 *@{
                 */
                case Tags.EMAIL_IMPORTANCE:
                    if (UIProvider.ENABLE_MAIL_PRIORITY_FEATURE) {
                        int mailPriority = getValueInt();
                        if (mailPriority == UIProvider.MailImportance.HIGH) {
                            msg.mFlags |= EmailContent.Message.FLAG_TYPE_PRIORITY_HIGH;
                        } else if (mailPriority == UIProvider.MailImportance.LOW){
                            msg.mFlags |= EmailContent.Message.FLAG_TYPE_PRIORITY_LOW;
                        }
                    }
                    break;
                /*@}*/
    ...
    private void attachmentParser(final ArrayList<EmailContent.Attachment> atts,
        ...
        if ((fileName != null) && (length != null) && (location != null)) {
            ...
            att.mMimeType = AttachmentUtilities.inferMimeType(fileName,null);//getMimeTypeFromFileName(fileName);
    ...
    public void commit() throws RemoteException, OperationApplicationException {
        /* SPRD: Modify for bug691532 @{ */
        int currentOpsPerBatch = MAX_OPS_PER_BATCH;
        do {
            try {
                commitImpl(currentOpsPerBatch);
                break;
                ...
                LogUtils.w(TAG, e,
                        "Transaction too large, retrying in half mode, currentOpsPerBatch = %d",
                        currentOpsPerBatch);
                currentOpsPerBatch /= 2;
            }
        } while (currentOpsPerBatch >= 1);
        /* @} */
    }
